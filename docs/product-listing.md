...
`auction: None` means no reliable auction-participation assertion. `Some(ProductListingAuction)` asserts auction participation even when its membership, lot number, catalogue position, and timing are all absent. A resolved membership carries one same-source `AuctionId`; `None` membership remains explicitly unresolved. Lot number is an opaque 1–128-byte source label; catalogue position is an optional positive one-based `u32`. `LotAuctionTiming` has independently optional `bidding_opens`, `scheduled_closes`, and exact `reported_closed_at`; the first two retain `INSTANT` or source `DATE` precision, while date-only values never become midnight. Comparable exact bounds and source dates with the same declared timezone reject an opening after its scheduled close.

Create accepts `Option<ListingAvailability>`; omitted and JSON `null` create an active listing without an assertion. Main price transport is tagged `MONETARY { currency, amount }` or `ON_REQUEST`; only `MONETARY` may be converted, formatted as money, or match numeric filters. Update and upsert use tri-state patches for availability, main price, and each price estimate: omitted is unchanged, `null` clears, and a value sets. The outer typed partner `auction` context is omitted to preserve it or supplied as an assertion with nested patches: missing lot fields preserve, explicit nullable lot fields clear, and `sourceAuctionId` is non-null/non-clearable (omit to preserve membership; supply a key to resolve it). `{}` participates as an empty context assertion. Complete context replacement belongs to administrative correction; outer `null` and inner `sourceAuctionId: null` are rejected because correction owns retraction. Correction/release currently has release-blocking SQL/fencing defects; see [iteration 12](auction-iterations/12-release-audit.md). Raw `auction: CLEAR` preserves an existing context and yields no detach; raw source-key `CLEAR` also preserves it with a durable correction diagnostic. On creation, omitted or `null` auction means no assertion, while `{}` asserts an empty context. Upsert images are separate: omitted preserves existing images, `[]` clears them, and `null` is invalid. URL is non-clearable: omitted or `null` preserves it; a URL value sets it. Existing withdrawn listings are restored by explicit upsert intent before current facts are applied.
...
`product-service::NormalizeProductListingRawRevisionUseCase` is the sole authoritative raw-to-canonical path. A listing-owned Auction override policy is read in this transaction: an active override preserves Auction context while unrelated valid facts continue; after explicit release, revisions captured at or before the durable global capture-generation floor remain unable to apply Auction context. Policy-only correction/release writes do not advance ProductListing aggregate, current-event, or projection revisions and append no ProductListing event.  It locks one `product_listing_raw_normalization_heads` stream head, processes only its immediate next revision, writes one immutable `product_listing_raw_normalizations` terminal result with ordered bounded diagnostics and optional raw-revision-to-Auction acceptance evidence, and advances the head in the same PostgreSQL transaction as zero or one canonical ProductListing event. A reliable Auction acceptance records its Auction ID, resulting version, optional event, and `CREATED`, `METADATA_APPLIED`, or `NO_CHANGE` disposition. Optional raw Auction reference, lot, timing, metadata, and membership-conflict failures suppress only their Auction group while independently valid listing facts continue; structural raw-contract and system failures remain strict. `APPLIED`, `NO_CHANGE`, `IGNORED`, and `REJECTED` advance a stream. Candidate-data normalization failures become terminal `REJECTED`; a `System` normalizer configuration failure, including availability-regex compilation, returns retryable `NORMALIZATION_CONFIGURATION_FAILED` before a transaction or completion and leaves the raw revision and stream head pending. Deployment/schema mismatches and transient persistence failures also leave it pending. The `product-listing-normalization` worker scope accepts only committed `product_listing_raw_revisions` inserts, carries typed stream/revision IDs only, and drains streams in order. CDC wake-ups are durably published to scoped Standard SQS before Sequin acknowledgment. Startup and periodic bounded reconciliation also repair missed wake-ups from authoritative pending raw revisions; only its reconstructible cursor/continuation FIFO remains in memory. Capture and normalization emit metadata-only outcome, count, size, latency, and bounded-backlog signals; the operator queries are in `docs/product-listing-raw-normalization-runbook.md`.
