...
A ProductListing is identified by `(ListingSourceId, SourceListingId)`. `SourceListingId` is an opaque partner value: Aura trims outer Unicode whitespace, rejects blank values and embedded NUL characters, preserves case, punctuation, and internal whitespace, and accepts at most 512 UTF-8 bytes. The trimmed value is the canonical input and the only value persisted in PostgreSQL, used for the authoritative key, emitted in events, and used to derive the source-scoped URL ID; pre-trim input is not retained. It has no seller, auctioneer, Party attribution, address, or location state. The created event includes both immutable source identifiers. Actor attribution belongs to #1321; durable raw input to #1646; addresses to #1635.

`SourceListingSlugId` is a separate immutable Aura-owned URL identity, scoped by `ListingSourceSlugId`. Aura derives it from the canonical `SourceListingId` as a capped ASCII slug body plus the first 20 lowercase hexadecimal characters (80 bits) of its SHA-256 digest. The full raw ID is hashed before only the readable body is truncated; the locator is at most 255 bytes and falls back to `listing` when no ASCII body remains. The digest suffix makes normalization collisions deterministic and distinct. PostgreSQL retains the canonical `(listing_source_id, source_listing_id)` key as authoritative and additionally enforces unique `(listing_source_id, source_listing_slug_id)`. Public detail and summary representations issue both raw `sourceListingId` and `sourceListingSlugId`; clients compose `source.slugId` as `listingSourceSlugId` with the issued locator for `GET /api/v1/listing-sources/{listingSourceSlugId}/product-listings/{sourceListingSlugId}`. The route does not accept the provider key in its path.
...
The initial schema uses `product_listings`, `product_listing_events`, `product_listing_translations`, and `product_listing_watchlist`; IDs use `product_listing_id`, `product_listing_slug_id`, `listing_source_id`, `source_listing_id`, and `source_listing_slug_id`. `product_listings` retains the unique canonical `(listing_source_id, source_listing_id)` key, adds unique `(listing_source_id, source_listing_slug_id)` URL lookup, and has a cascading foreign key to `listing_sources`.
