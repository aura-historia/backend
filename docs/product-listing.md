...
`product-service::NormalizeProductListingRawRevisionUseCase` is the sole authoritative raw-to-canonical path. It preserves stored Auction/lot facts because raw input has no Auction fields. It locks one `product_listing_raw_normalization_heads` stream head, processes only its immediate next revision, writes one immutable `product_listing_raw_normalizations` terminal result with ordered bounded diagnostics, and advances the head in the same PostgreSQL transaction as zero or one canonical ProductListing event. `APPLIED`, `NO_CHANGE`, `IGNORED`, and `REJECTED` advance a stream. Candidate-data normalization failures become terminal `REJECTED`; configuration, deployment/schema, and transient persistence failures leave the revision and stream head pending. The dedicated `product-listing-normalization-lambda` accepts only committed `product_listing_raw_revisions` inserts, carries typed stream/revision IDs only, and drains streams in order. CDC wake-ups are durably published to scoped Standard SQS before Sequin acknowledgment. The Lambda acknowledges only a fully drained stream and returns every capped or otherwise unfinished attempt as `batchItemFailures`; it never depends on a warm cursor or FIFO. Bounded scheduled reconciliation repairs missed wake-ups from authoritative pending raw revisions and must be live before the native reconciliation timer is retired. Capture and normalization emit metadata-only outcome, count, size, latency, and bounded-backlog signals; the operator queries are in `docs/product-listing-raw-normalization-runbook.md`.
