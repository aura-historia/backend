...
`product-service::NormalizeProductListingRawRevisionUseCase` is the sole authoritative raw-to-canonical path. It preserves stored Auction/lot facts because raw input has no Auction fields. It locks one `product_listing_raw_normalization_heads` stream head, processes only its immediate next revision, writes one immutable `product_listing_raw_normalizations` terminal result with ordered bounded diagnostics, and advances the head in the same PostgreSQL transaction as zero or one canonical ProductListing event. `APPLIED`, `NO_CHANGE`, `IGNORED`, and `REJECTED` advance a stream. Candidate-data normalization failures become terminal `REJECTED`; configuration, deployment/schema, and transient persistence failures leave the revision and stream head pending. The dedicated `product-listing-normalization-lambda` accepts only committed `product_listing_raw_revisions` inserts, carries typed stream/revision IDs only, and drains streams in order. In the target path, DMS/Kinesis/router delivery durably publishes CDC wake-ups to scoped Standard SQS. The Lambda acknowledges only a fully drained stream and returns every capped or otherwise unfinished attempt as `batchItemFailures`; it never depends on a warm cursor or FIFO. Target recovery is bounded authoritative stream draining plus ordinary SQS retry/redrive and persisted PostgreSQL progress; it has no scheduled raw-normalization reconciliation. Capture and normalization emit metadata-only outcome, count, size, latency, and bounded-backlog signals; [durable-worker operations](durable-worker-runbook.md#failure-custody-and-controlled-redrive) covers pending heads, wake-up loss, and controlled recovery. Crawler-local retry and dormancy remain governed by the [crawler scheduling contract](crawler/scheduling-and-retries.md).

The initial schema uses `product_listings` with listing-owned optional Auction/lot columns, `product_listing_events`, `product_listing_translations`, and `product_listing_watchlist`; IDs use `product_listing_id`, `product_listing_title_slug_id`, `listing_source_id`, and `source_listing_id`. `product_listings` retains the unique canonical `(listing_source_id, source_listing_id)` key for partner writes, globally enforces unique `product_listing_title_slug_id` for public lookup, and has a cascading foreign key to `listing_sources`. Withdrawal is reversible and retains a watch row unchanged: its state, quota occupancy, and current-interval timestamps survive withdrawal and restore. An explicit physical ProductListing delete cascades listing-owned translations, events, content assessments, watchlist rows, and search-filter matches, but not immutable notification snapshots or their delivery rows. The initial schema rewrite is direct: no outbox, compatibility decoder, migration, backfill, or dual write exists.
