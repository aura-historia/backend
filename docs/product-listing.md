...
A ProductListing is identified by `(ListingSourceId, SourceListingId)`. `SourceListingId` is an opaque partner value: Aura trims outer Unicode whitespace, rejects blank values, preserves case, punctuation, and internal whitespace, and accepts at most 512 UTF-8 bytes. The trimmed value is the canonical input and the only value persisted in PostgreSQL, used for the authoritative key, emitted in events, and used to derive the source-scoped URL ID; pre-trim input is not retained. It has no seller, auctioneer, Party attribution, address, or location state. The created event includes both immutable source identifiers. Actor attribution belongs to #1321; durable raw input to #1646; addresses to #1635.

`SourceListingSlugId` is a separate immutable URL identity, scoped by `ListingSourceSlugId`. Aura derives it from the canonical `SourceListingId` as an ASCII slug body plus the first 12 lowercase hexadecimal characters of its SHA-256 digest. The digest suffix makes normalization collisions deterministic and distinct. PostgreSQL retains the canonical `(listing_source_id, source_listing_id)` key as authoritative and additionally enforces unique `(listing_source_id, source_listing_slug_id)`. Public source-scoped detail reads use `GET /api/v1/listing-sources/{listingSourceSlugId}/product-listings/{sourceListingSlugId}`; they do not accept the provider key in the path.
