...
`auction: None` means no auction-participation assertion. `Some(ProductListingAuction)` asserts participation even when membership, lot number, catalogue position, and timing are absent. Membership carries one same-source `AuctionId`. Lot number is an opaque 1–128-byte source label; catalogue position is an optional positive one-based `u32`. `LotAuctionTiming` has independently optional `bidding_opens`, `scheduled_closes`, and exact `reported_closed_at`; the first two retain `INSTANT` or source `DATE` precision, while date-only values never become midnight.
