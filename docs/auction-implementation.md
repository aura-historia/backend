...
**Status:** release INCOMPLETE. [Iteration 12 audit](auction-iterations/12-release-audit.md) repaired the reproduced correction SQL/fencing and OpenAPI contract defects, but the complete acceptance matrix and coordinated reset/recapture rehearsal remain unverified. Earlier command PASS records remain historical evidence, not release approval.
...
| Source deletion | `listing-source-service`/`listing-source-postgres` block source deletion for retained ProductListings, raw streams, and partnership-application references. Retained Auctions also block deletion, including ID-only, cancelled, and ended records. |
...
| 06 | Source-key membership and transactional resolution | **INCOMPLETE (audit A12-04).** `auction-*`, listing context persistence/events, canonical writer, `product-service`, direct partner path, and projection membership mapping. Correction barriers, crawler extraction, public Auction reads, and Auction-ID filters remain later iterations. |
| 07 | Corrections, override barrier, and safe release | **INCOMPLETE (audit A12-04).** Listing-owned policy/audit/floors, canonical/raw/partner guards, admin context/correction/release endpoints, strict `If-Match`, and historical full worker verification exist. Real PostgreSQL regression and black-box policy-only correction/release tests pass; full raw/partner/admin race acceptance remains. |
| 08 | Fixture-backed crawler auction extraction | **INCOMPLETE (audit A12-04 and prerequisite 07).** `crawler` maps the fixture-backed Lot-tissimo source key, catalogue URL, reviewed catalogue-name/lot-label selectors, and nonblank date-only labelled lot opening/close into the current raw Auction patch. It preserves no-key participation separately from resolved membership and adds no canonical crawler write. A real PostgreSQL raw capture → normalizer → resolver test proves participation followed by reliable-key resolution; broader source coverage remains unverified. |
| 09 | Batched Auction summaries on listing reads | **INCOMPLETE (audit A12-04).** `auction-service` batch-reader port, `auction-postgres` adapter, and ProductListing detail/search/similar/watchlist presentation return safe current Auction summaries without shared-metadata indexing or fan-out. |
| 10 | Public browsing | **INCOMPLETE (audit A12-04).** Auction directory/detail/catalogue readers and public controllers exist. OpenAPI schemas now parse; populated presentation acceptance and query-count gaps remain. |
| 11 | Auction-ID search and saved search | **INCOMPLETE (audit A12-04).** ProductListing query/codec/tier paths exist; historical API/worker/OpenSearch results are recorded. Auction-specific persisted saved-search acceptance remains unverified. |
| 12 | Release audit | **INCOMPLETE.** The correction SQL/fencing and OpenAPI faults are repaired and focused regression/HTTP tests pass. Full acceptance and coordinated reset/recapture remain unverified. See the audit for exact commands, coverage, and required owning fixes. |

Dependencies point from `composition -> auction-postgres -> auction-service -> auction-core`, with `product-listing-core` depending only on Auction semantic values required by the final context. Auction service/core must not depend on ProductListing service/core. Catalogue ownership stays in `product-listing-service`; presentation reads use bounded readers, not repositories.
...
| Manual correction policy | absent | 07 | policy/audit/floors, both ingestion guards, admin ETag flows, transaction-scoped policy writes, and full worker verification. Focused PostgreSQL and policy-only HTTP acceptance pass; raw/partner race acceptance remains. |
| Crawler reference extraction | absent | 08 | selector schema/review/output and fixture-backed provider rules; no direct canonical write; durable raw-to-resolution coverage. |
...
No single repository command for a coordinated development business-schema/raw-queue/index reset was found during this inventory. Later persisted/wire-contract iterations must name the reviewed, authorized tooling and exact dependency order before claiming a reset rehearsal. Iteration 12 used only process-isolated test setup/teardown. It performed no shared reset, deployment, remote purge, or coordinated development cutover.
