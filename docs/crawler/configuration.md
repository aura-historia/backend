...
| `spider_batch_size`              | `i64`         | 10            | Max spider candidates selected per scheduler refill query (`LIMIT` in candidate query); not a scheduler-pass completion barrier                                                                                                                                                                                                                                                         |
| `scraper_batch_size`             | `i64`         | 100           | Max scraper URLs selected per scheduler refill query; returned URLs are grouped by host before being assigned to domain-group slots                                                                                                                                                                                                                                                     |
...
| `spider_concurrency`             | `usize`       | 3             | Number of global spider domain slots kept busy during a scheduler pass                                                                                                                                                                                                                                                                                                                 |
| `scraper_concurrency`            | `usize`       | 10            | Number of global scraper domain-group slots kept busy during a scheduler pass                                                                                                                                                                                                                                                                                                          |
...
- **`min_inference_sample_urls`**: URL pattern inference is not attempted when the crawl sample has fewer than this many
  ...
  URL fail as `TinyCrawl`, and crawls with 2-19 URLs fail as `InsufficientInferenceSample`. These failure modes avoid
  LLM
  ...
  When spider library diagnostics identify a clearer cause for a zero/one-page crawl, the crawler persists that specific
  kind instead, such as `RateLimited`, `AccessDenied`, `CloudflareChallenge`, `BotProtection`, `TlsError`,
  `ConnectError`, `ServerError`, `RedirectProblem`, `InvalidUrl`, or `JavascriptRequired`. These persisted values are
  stable operational buckets owned by the crawler, not a 1:1 copy of `spider` internal enums or anti-bot vendors. They
  have their own cooldown groups based on whether the likely cause is transient, recoverable, or a durable block.

- **Spider crawl politeness**: The spider wrapper delegates robots.txt parsing, robots cache, `Crawl-delay`,
  retry/status
  handling, and adaptive crawl delay to the `spider` crate. `delay_millis` remains the baseline delay and the minimum
  adaptive throttle delay.

- **`spider_batch_size` + `spider_concurrency`**: Each scheduler pass keeps up to `spider_concurrency` domain slots
  active. `spider_batch_size` caps each refill query. When a domain finishes, the scheduler immediately fetches more
  eligible candidates excluding domains already in flight or skipped in the pass. If a domain's in-memory lock
  (`DomainLock`) is held by another task the item is skipped immediately rather than failing.

- **`scraper_batch_size` + `scraper_concurrency` + `scraper_domain_delay`**: Each scheduler pass keeps up to
  `scraper_concurrency` domain-group slots active. `scraper_batch_size` caps each refill query; returned URLs are grouped
  by host, and each domain worker processes its URLs sequentially. The scheduler avoids running the same host in two
  slots in the same pass. The real HTML fetcher applies
  latency-aware per-domain auto-throttling before each HTTP request: `scraper_domain_delay` is the minimum delay, the
  observed fetch latency is smoothed with an EMA (`alpha = 0.15`), and the next delay is approximately
  `latency / target_concurrency` with `target_concurrency = 2`, clamped to 10 seconds. Retryable URL failures still
  write URL-level `next_retry_at`; `429` responses with `Retry-After` are respected by the fetcher's retry loop.
  Throttle state is in-memory and process-local.
