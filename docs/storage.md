...
- In the current legacy path, the generic delivery worker receives committed `notification_deliveries` INSERT rows through a Sequin subscription; Sequin is acknowledged only after confirmed publication to the scoped Standard SQS queue. The target shared DMS/Kinesis router instead routes these inserts to the same queue after approved cutover, without a per-worker Sequin subscription. The worker claims a lease, dispatches by channel, and finalizes that same lease. EMAIL is the only valid stored channel now; a future channel needs a core enum and schema migration plus its sender. EMAIL resolves its current target and performs S3/SES I/O after the claim transaction commits. A send/finalize crash can duplicate an external delivery, so delivery is at-least-once, not exactly-once. Custody is bounded by source7d/DLQ14d retention. See the [durable-worker runbook](durable-worker-runbook.md).
