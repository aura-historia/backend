...
`product_listings` remains the authoritative ProductListing write model. Optional asserted auction context is owned by `product_listing_auction_contexts`; its optional timing row is `product_listing_lot_auction_timings`. Row absence means no auction-participation assertion, while a context row with all nullable leaves empty is an asserted empty context. These child rows are written and version-fenced only through the root ProductListing repository. Timing retains `INSTANT` versus `DATE` precision plus optional validated source timezone; `reported_closed_at` is exact only. Context rows have optional `auction_id` and a composite same-source Auction foreign key. Listing-owned `product_listing_auction_overrides`, `product_listing_auction_override_floors`, `product_listing_auction_corrections`, and `product_listing_auction_override_releases` store independent policy/audit state. Auction-context writers, corrections, and release acquire one transaction-scoped PostgreSQL advisory lock per ProductListing before policy/listing expectations are read. The lock covers absent policy rows and serializes policy-only correction/release against ordinary Auction context writes. Policy-only mutations must not advance listing domain/projection revisions. Regression and release evidence is recorded in [iteration 12](auction-iterations/12-release-audit.md).
