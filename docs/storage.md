...
- `search_filter_matches.price_valuation_basis` and `price_fx_rate_id` are null together for non-price matches. Price matches persist `EVENT` with the event-effective snapshot or `SALE` with the immutable sale snapshot. The FX ID references immutable `fx_rates`.
