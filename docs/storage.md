...
- In the post-legacy target, the shared DMS/Kinesis router routes committed `notification_deliveries` INSERTs to the scoped Standard SQS queue after approved activation; there is no per-worker subscription. Historically the native worker ingested these through Sequin; deleting its source does not prove live external Sequin decommissioning. The worker claims a lease, dispatches by channel, and finalizes that same lease. EMAIL is the only valid stored channel now; a future channel needs a core enum and schema migration plus its sender. EMAIL resolves its current target and performs S3/SES I/O after the claim transaction commits. A send/finalize crash can duplicate an external delivery, so delivery is at-least-once, not exactly-once. Custody is bounded by source7d/DLQ14d retention. See the [durable-worker runbook](durable-worker-runbook.md).
- Initial business schema defines paired `completed_lease_token`/`completed_at`. Finalization fences the active lease, then recognizes an exact committed token/result/time receipt on retry without rewriting it. Reclaim clears that receipt; terminal rows created before a completion remain null, never with invented tokens.
...
- Expiry remains service correctness. Plain `expires_at` indexes support `cleanup_expired_credentials_and_provider_receipts(batch_size)`, a physical-only PostgreSQL operation. It deletes at most 1,000 already-expired rows per target invocation with `FOR UPDATE SKIP LOCKED`; callers commit each call before starting another. Non-expiring access tokens have `expires_at IS NULL`.
- Credential tables are operational only. They must not enter DMS CDC selection, historical Sequin subscriptions, projections, analytics, or credential-bearing logs. The raw token in a third-party exchange-code row is short-lived escrow needed by that exchange only.
...
- Canonical ProductListing identity uses the unique `(listing_source_id, source_listing_id)` key. `url` has no full `(listing_source_id, url)` B-tree, so valid long canonical URLs remain storable; withdrawal uses canonical ID or this source-listing key.
