...
`product-listing-opensearch-lambda`, `product-listing-normalization-lambda`, `search-filter-projection-lambda`, and `search-filter-percolator-lambda` consume their respective queues only after independent explicit activation. The other six consumers remain composed in `aura-historia-worker` for now. Periodic matching stays in `aura-historia-cron`, not another queue scope.
