...
`product-listing-opensearch-lambda`, `product-listing-normalization-lambda`, `product-content-assessment-lambda`, `search-filter-projection-lambda`, `search-filter-percolator-lambda`, `search-filter-match-notification-lambda`, `watchlist-notification-lambda`, and `notification-delivery-lambda` consume only their respective queues after independent explicit activation. The other two consumers remain composed in `aura-historia-worker` for now. Periodic matching stays in `aura-historia-cron`, not another queue scope.
