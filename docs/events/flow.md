...
| Scoped Standard SQS source/DLQ pairs | Durable transport | One pair per scope; ProductListing OpenSearch, ProductListing normalization, saved-filter projection/percolator, and notification delivery have retained dedicated Lambda mappings disabled until explicit activation; source7d/DLQ14d retention and duplicate/reordered delivery apply. |
...
The native `aura-historia-worker` router uses a scope-specific fanout and publishes only its scope's queue. Its catalog has ten production scopes: `product-listing-opensearch`, `product-listing-normalization`, `search-filter-projection`, `search-filter-percolator`, `search-filter-match-notification`, `watchlist-notification`, and `notification-delivery` have retained dedicated Lambda mappings, each independently disabled until explicit activation; the remaining three consumers stay native. There is no user-tier scope or tier dimension, worker inbox, or processed-job table. Actual process/Sequin deployment remains externally owned.
...
`product-listing-opensearch-lambda`, `product-listing-normalization-lambda`, `product-content-assessment-lambda`, `product-embedding-lambda`, `search-filter-projection-lambda`, `search-filter-percolator-lambda`, `search-filter-match-notification-lambda`, `watchlist-notification-lambda`, and `notification-delivery-lambda` consume only their respective queues after independent explicit activation. Translation remains composed in `aura-historia-worker` for now. Periodic matching stays in `aura-historia-cron`, not another queue scope.
