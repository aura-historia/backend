...
| Scoped Standard SQS source/DLQ pairs | Durable transport | One pair per scope; ProductListing OpenSearch, raw normalization, and saved-filter percolator have retained dedicated Lambda mappings disabled until explicit activation, others remain native; source7d/DLQ14d retention and duplicate/reordered delivery apply. |
...
`product-listing-opensearch-lambda`, `product-listing-normalization-lambda`, and `search-filter-percolator-lambda` consume their respective queues only after independent explicit activation. The other seven consumers remain composed in `aura-historia-worker` for now. Periodic matching stays in `aura-historia-cron`, not another queue scope.
