...
|---|---|---|
| `product_listing_events` | INSERT | Projector for every supported event; percolator for every domain/enrichment event; content assessment and embedding for `PRODUCT_LISTING_DISCOVERED`; watchlist for `PRODUCT_LISTING_CHANGED` only when payload has main-price or availability changes; translation for `ENRICHMENT_EMBEDDED`. Lifecycle is a changed-event dimension, not an event group. |
...
| ProductListing embed | legacy `product-pipeline-embed-text` | `PRODUCT_LISTING_DISCOVERED` job | Postgres enrichment event + ProductListing update. Embedding stored in Postgres only. |
...
Before writing, the use case locks the ProductListing row with `FOR SHARE` and rechecks `product_listings.current_event_id = product_listing_events.event_id`. The lock remains held through notification and delivery-intent insertion and transaction commit. Missing or stale ProductListing-event sources are acknowledged suppression outcomes, not retryable failures. `GenerateWatchlistNotificationsResult::Applied`, `SuppressedForMissingSource`, and `SuppressedForStaleProductListingEvent` are successful outcomes; worker logs additionally distinguish applied work from duplicate writes.
