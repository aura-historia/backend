...
| Scoped Standard SQS source/DLQ pairs | Durable transport | One pair per scope; ProductListing OpenSearch, ProductListing normalization, saved-filter projection, and saved-filter percolator have retained dedicated Lambda mappings disabled until explicit activation; source7d/DLQ14d retention and duplicate/reordered delivery apply. |
...
The native `aura-historia-worker` router uses a scope-specific fanout and publishes only its scope's queue. Its catalog has ten production scopes: `product-listing-opensearch`, `product-listing-normalization`, `search-filter-percolator`, `search-filter-match-notification`, and `watchlist-notification` have retained dedicated Lambda mappings, each independently disabled until explicit activation; the remaining consumers stay native. There is no user-tier scope or tier dimension, worker inbox, or processed-job table. Actual process/Sequin deployment remains externally owned.
...
`product-listing-opensearch-lambda`, `product-listing-normalization-lambda`, `search-filter-projection-lambda`, `search-filter-percolator-lambda`, `search-filter-match-notification-lambda`, and `watchlist-notification-lambda` consume only their respective queues after independent explicit activation. The other four consumers remain composed in `aura-historia-worker` for now. Periodic matching stays in `aura-historia-cron`, not another queue scope.
