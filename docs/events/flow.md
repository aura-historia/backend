...
| Scoped Standard SQS source/DLQ pairs | Durable transport | One pair per scope; ProductListing OpenSearch, saved-filter projection, and ProductListing normalization have retained Lambda mappings disabled until explicit activation; source7d/DLQ14d retention and duplicate/reordered delivery apply. |
...
`product-listing-opensearch-lambda`, `search-filter-projection-lambda`, and `product-listing-normalization-lambda` consume their respective queues only after independent explicit activation. The other seven consumers remain composed in `aura-historia-worker` for now. Periodic matching stays in `aura-historia-cron`, not another queue scope.
