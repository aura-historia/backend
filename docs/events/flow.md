...
| Scoped Standard SQS source/DLQ pairs | Durable transport | One pair per scope; all scoped consumers have retained dedicated Lambda mappings disabled until explicit activation; source7d/DLQ14d retention and duplicate/reordered delivery apply. |
...
flowchart TD
    ...
    NQ --> ND_LAMBDA["Notification delivery Lambda"]
    ND_LAMBDA -->|"claim, send, finalize"| PG
    ND_LAMBDA --> SES
...
The native `aura-historia-worker` router uses a scope-specific fanout and publishes only its scope's queue. Its catalog has ten production scopes, each with a retained dedicated Lambda mapping independently disabled until explicit activation. There is no user-tier scope or tier dimension, worker inbox, or processed-job table. Actual process/Sequin deployment remains externally owned.
...
3. Parse PostgreSQL/CDC UUID text into typed UUIDv7 IDs, then publish compact schema-2 jobs with TypeID object fields and domain-first `idempotency_key`/`ordering_key`. Unknown additive SQS envelope/payload fields are tolerated; required fields, schema/type/scope/IDs and keys remain strict. Schema 1 is rejected without compatibility decoding. This does not loosen upstream event validation.
...
Normalization reconciliation is an authoritative bounded PostgreSQL traversal. Its cursor is reconstructible and only a native development fixture may retain local scheduling hints; the Lambda never depends on warm process memory. Scheduled reconciliation remains a rollout gate before retiring the native timer; see the [normalization runbook](../product-listing-raw-normalization-runbook.md).
...
| `product_listing_raw_revisions` | INSERT | `product-listing-normalization` only. Compact raw stream/revision IDs and revision number; never source JSON. The dedicated SQS Lambda drains the PostgreSQL stream head in order and atomically writes terminal raw progress with zero or one canonical state/event. Candidate-data failures are terminal `REJECTED`; configuration/persistence failures leave progress pending. Only a fully drained stream is acknowledged; capped direct drains and all unfinished failures return `batchItemFailures`. Bounded authoritative reconciliation uses a reconstructible cursor and is scheduled separately. No direct projection, notification, enrichment, assessment or matching route. |
...
| ProductListing normalization | dedicated SQS Lambda | Raw revision job plus scheduled authoritative reconciliation | Atomic canonical state/event and raw progress; only clean drains acknowledge and no Lambda-local continuation state is required. |
...
`watchlist-notification-lambda` uses native Lambda SQS delivery after `WatchlistNotificationConsumerEnabled=true`, with batch size one, `ReportBatchItemFailures`, a 45-second invocation limit, and 300-second source visibility. It has only `POSTGRES_*` configuration and source-queue receive/delete/visibility permissions: no OpenSearch, SES, template, or direct publish access. Applied, duplicate, ignored, and withdrawn results acknowledge; missing committed sources, dependency failures, malformed records, timeout, panic, and unknown commit remain failed for retry/DLQ. Its CDC subscription accepts only `product_listing_events` inserts; the default `search-filter-projection` scope remains separately subscribed to `search_filters`.
...
| FX bootstrap and refresh — protected release / dedicated Lambda | Protected `Initialize (CD)` invokes `fxrate-lambda` once after business migrations. The real-stage EventBridge rule subsequently invokes the same Lambda at `cron(0 6,18 * * ? *)`; ephemeral has neither function nor schedule. | Bootstrap requires the CI deploy role's exact `lambda:InvokeFunction` grant; the stable deployment event ID gives idempotency. Scheduled events are EventBridge events, not a public endpoint. | Initialization stops before activation when migration or initial capture fails. The recurring rule allows a one-hour event age and three retries; it has no failure destination. Snapshot idempotency is `fx_rates.source_event_id`. | FX Lambda has only the runtime PostgreSQL secret plus stage-specific `/fxratesapi/<stage>/api-token` configuration. It runs privately with verified TLS, a lazy max-one reusable pool, and warm secret-version composition refresh. |
...
`PartnerIntegrationEnabled` independently enables the retained Shopify SQS mapping and both Stripe/Shopify EventBridge rules. `FxRateRefreshEnabled` independently enables only the recurring FX rule. Both default to `false`; protected initialization deploys them disabled, runs migration then the idempotent first FX capture, and only then enables them. This prevents a normal ProductListing worker handoff from enabling an unrelated provider or schedule and preserves the required initialization order. Normal deployment leaves the selected activation state unchanged.
