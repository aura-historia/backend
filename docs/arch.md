...
OpenSearch runtime composition uses `platform-opensearch` for shared client TLS mechanics, not domain queries or environment lookup. `dev`/`prod` require HTTPS and `OPENSEARCH_SSL_ROOT_CERT`; only explicit `local`/`test`/`ephemeral` may omit the CA or use HTTP. Supplied CA with HTTP is invalid. Validate/freeze a bounded certificate-only PEM bundle before dependency I/O; certificate/hostname verification stays enabled. Roots augment built-in trust, not exclusive pinning; rotation requires rebuilding clients. Both SDK and direct preflight disable proxies. Pinned SDK2.4 has no redirect-policy setter and retains redirects (including possible destination/scheme changes); the direct worker preflight rejects redirects. Do not advertise SDK transport as HTTPS-only across redirects; use a trusted nonredirecting search endpoint. No SDK fork or general networking framework is introduced for this CA input.
