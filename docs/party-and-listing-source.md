...
Create uses an explicit operator input: `EXISTING` carries `partyId`; `NEW` carries Party name and optional contact. Admins can search Party summaries and create Parties through `GET`/`POST /api/v1/admin/parties`; search uses bounded cursor pagination and name/contact filters, while create returns the stable identity and immutable slug. Party item and update routes are separate work; there is no ListingSource search/list-all route. Public contract details are in `docs/swagger.yaml`.
