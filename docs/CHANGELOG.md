...
- **Breaking:** Partner ProductListing create, update, and upsert Auction contexts now apply nested leaf patches. In an asserted `auction`, omitted lot/timing leaves preserve current facts, `null` clears clearable leaves, and values set them; `{}` is an empty-context assertion. `sourceAuctionId` is non-null/non-clearable: omit it to preserve membership or provide a reliable key to resolve it. Outer `auction: null` and inner `sourceAuctionId: null` are invalid. Raw outer Auction `CLEAR` and raw source-key `CLEAR` remain non-destructive pending explicit correction, with the latter recording `MEMBERSHIP_CHANGE_REQUIRES_CORRECTION`. No legacy replacement shape or compatibility decoder is accepted.

- **Breaking:** Raw ProductListing Auction normalization now isolates optional Auction reference, lot, timing, metadata, and A-to-B membership-conflict losses so unrelated valid listing facts commit with ordered bounded durable diagnostics. Typed partner writes remain atomic on membership conflict; malformed current raw contracts and system failures remain strict. Every accepted raw reliable Auction reference now records transactional raw-revision receipt evidence with Auction ID, result version, optional event, and `CREATED`, `METADATA_APPLIED`, or `NO_CHANGE` disposition. No compatibility reader, dual write, or backfill exists.

- OpenAPI now parses with coherent saved-filter YAML aliases. Public Auction `GET /api/v1/auctions`, `GET /api/v1/auctions/{auctionId}`, and `GET /api/v1/auctions/{auctionId}/product-listings` success bodies are explicitly documented from their controller DTOs. Partner ProductListing `ProductListingAuctionData` remains write-only; public ProductListing detail and immutable history now use a separate safe Auction context that exposes only optional resolved `auctionId`, lot fields, and timing, never `sourceAuctionId` or metadata.

- ProductListing OpenSearch search and saved-search filters now accept up to 100 repeated exact resolved-membership `auctionId` values. IDs use strict `auc_` TypeIDs, OR together, and intersect all other criteria before pagination; unresolved and no-context listings do not match. Saved-filter persistence and percolation use the same predicate, and the filter has the same tier restriction as `listingSourceId`. No Auction index or shared-metadata fan-out was added.

- Public Auction browsing is available at `GET /api/v1/auctions`, `GET /api/v1/auctions/{auctionId}`, and `GET /api/v1/auctions/{auctionId}/product-listings`. These authoritative PostgreSQL reads use strict `auc_` IDs and `Cache-Control: no-store`. Directory paging is bounded (default 21, maximum 100) in fixed newest-first order and its optional `timeRole`/`from`/`to` filter matches only exact schedule instants in half-open ranges; returned cursors are scoped to the complete filter set. Detail keeps source `reportedLotCount` distinct from current `visibleListingCount`; an ID-only Auction and an empty visible catalogue return `200`. Catalogue returns active assigned listings only, with `cataloguePosition ASC NULLS LAST` plus listing UUID ordering and an Auction-scoped cursor. Its `language` and `currency` parameters use normal personalized ProductListing detail presentation, including localization, FX valuation, referral URL, user state, content-policy image redaction, and lot timing.

- ProductListing detail, search, similar-listing, and watchlist responses now include optional `auctionSummary` for resolved Auction context. It contains only the safe current Auction ID, optional name, format, reported status, and explicit schedule; unresolved context remains without a fabricated summary. Listing search remains eventually consistent for listing-owned fields while the summary comes from authoritative PostgreSQL.

- Administrator ProductListing Auction-context correction is available at `GET /api/v1/admin/product-listings/{productListingId}/auction-context`, `POST /api/v1/admin/product-listings/{productListingId}/auction-corrections`, and `DELETE /api/v1/admin/product-listings/{productListingId}/auction-override`. All require persisted administrator authority, strict `pl_` IDs, `Cache-Control: no-store`, and strong `If-Match: "plv-{listing}-apv-{policy}"` for mutations. Correction fully replaces or retracts the context, requires a restricted nonblank reason, and activates an override even for equal facts. Raw Auction context is preserved while active; direct partner context writes conflict. Release changes only policy and retains a durable immutable raw-capture floor, so old captures cannot replay Auction context. Reasons, raw evidence, audit actor, and policy data are not exposed.

- **Breaking:** Partner ProductListing batch create, update, and upsert now use one nested `auction` context instead of flat `lotBiddingOpens` and `lotScheduledCloses`; the retired members are rejected. The context has optional `lotNumber`, one-based `cataloguePosition`, and optional `timing` with precision-tagged `biddingOpens` / `scheduledCloses` (`INSTANT` or source-timezone `DATE`) plus exact `reportedClosedAt`. Optional strict `auction.sourceAuctionId` resolves or creates shared Auction membership in the same listing transaction, and `auction.metadata` supplies fill-only Auction metadata. A normal update/upsert that asserts a different reliable key for an already-member listing returns `409 CONFLICT` and creates no new Auction; an explicit correction owns that change. Omit the outer context to preserve it; `null` is rejected. ProductListing detail and immutable history responses expose the optional lot context; detail emits `auction: null` when no context is asserted. Exact lot-time filters use half-open ranges and exclude date-only values; OpenSearch projects exact bidding-open, scheduled-close, and reported-close facts only.
...
- **Breaking (#1634, #1602):** Public ListingSource discovery is available at `GET /api/v1/listing-sources`. It supports anonymous browse and literal partial-name pages (default 21, maximum 50) with opaque `searchAfter` continuation; it is neither autocomplete nor fuzzy/full-text search. Every persisted ListingSource is eligible. Cards expose only ListingSource ID, immutable slug, display name, operator name, and optional safe presentation URL/image. The existing `GET /api/v1/listing-sources/by-slug/{listingSourceSlugId}` is now anonymous exact-slug lookup and returns that same public card rather than the old protected admin detail. Supplied invalid credentials still reject; admin ID detail, admin searches, and mutations stay protected. The initial schema directly installs and maintains `pg_trgm`/`unaccent` folded name representations; no API version, retained-data migration, backfill, or compatibility layer exists.
...
- **Breaking:** ProductListing raw UPSERT values now use one strict provider-neutral schema: persisted raw-values discriminator `1` and required `priceFormat`. `DISPLAY_TEXT` and `MACHINE_DECIMAL` retain their existing parsing behavior; crawler emits `DISPLAY_TEXT`, while Shopify and WooCommerce emit `MACHINE_DECIMAL`. Historical V1/V2 structs, discriminator `2`, and conversion branches are removed. Incompatible development raw revisions, queues, and fixtures must be reset and recaptured with matching code; they are not converted or compatibility-decoded.
- WooCommerce `price` uses current raw-values schema `1` `MACHINE_DECIMAL`: a nonblank `SET` is an untrimmed unsigned ASCII `digits` or `digits.digits` value with no whitespace, signs, grouping separators, or currency symbols. Fractional digits beyond the configured currency minor-unit scale are valid only when every excess digit is `0`; a nonblank `SET` requires configured `woocommerceCurrency`.
