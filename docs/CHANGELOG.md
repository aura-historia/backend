...
- OpenAPI now parses with coherent saved-filter YAML aliases. Public Auction `GET /api/v1/auctions`, `GET /api/v1/auctions/{auctionId}`, and `GET /api/v1/auctions/{auctionId}/product-listings` success bodies are explicitly documented from their controller DTOs. Partner ProductListing `ProductListingAuctionData` remains write-only; public ProductListing detail and immutable history expose safe listing-owned Auction/lot facts: optional resolved `auctionId`, lot fields, and timing, never `sourceAuctionId` or metadata.
...
- ProductListing detail, search, similar-listing, and watchlist responses now include optional `auctionSummary` for resolved Auction membership. It contains only the safe current Auction ID, optional name, format, reported status, and explicit schedule; missing or unresolved membership has no fabricated summary. Listing search remains eventually consistent for listing-owned fields while the summary comes from authoritative PostgreSQL.
