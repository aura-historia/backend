...
- ProductListing now exposes source-scoped detail lookup at `GET /api/v1/listing-sources/{listingSourceSlugId}/product-listings/{sourceListingSlugId}`. `sourceListingSlugId` is a deterministic slug of the preserved raw `sourceListingId` with a SHA-256 suffix; raw provider keys remain the authoritative PostgreSQL composite key.
- **Breaking:** Aura-owned listing routes use `/api/v1/product-listings`, `/api/v1/product-listings/{productListingId}`, `/api/v1/listing-sources/{listingSourceId}/product-listings`, and `/api/v1/me/watchlist/{productListingId}`. Listing IDs are `productListingId`, `productListingSlugId`, `listingSourceId`, and `sourceListingId`; storage and CDC use `product_listings`, `product_listing_events`, `product_listing_translations`, and `product_listing_watchlist`; errors use `PRODUCT_LISTING_*` and `LISTING_SOURCE_*`. No aliases remain.
