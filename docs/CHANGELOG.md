...
- **Breaking:** The legacy admin user detail route `GET /api/v1/users/{userId}` is removed; use `GET /api/v1/admin/users/{userId}`. The legacy admin PATCH route is removed; only legacy admin DELETE remains at `/api/v1/users/{userId}`.
