...
- Canonical Product search now pins its latest persisted FX snapshot and compiles display price ranges into exact native active-Product intervals. Sold Products use immutable sale-time target-currency values. Product and saved-filter OpenSearch projections no longer carry converted top-level prices or estimates; saved filters record their `compiledFxRateId`.
