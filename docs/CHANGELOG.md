...
- **Breaking:** User account responses no longer expose `structuredAddress`, and user account PATCH requests no longer accept it. User `geoAddress` remains available as an independent coordinate value. Structured-address-backed user country and continent filters are removed.
