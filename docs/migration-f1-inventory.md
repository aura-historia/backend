...
- **Starting default:** RDS PostgreSQL rather than Aurora; Lambda SQLx pools use min zero and max one per execution environment (API max two needs measured independent SQL concurrency); source retention 7d, DLQ retention 14d, redrive 5; one Kinesis shard; slow Lambda batches of one. Recalculate Lambda queue visibility from final Lambda timeout/batching rather than copying native-process values.
- **Implementation decision (#1779):** migrated roots require `POSTGRES_TLS_ROOT_CERT` and use SQLx `VerifyFull`, so plaintext, untrusted CAs, and wrong server identities fail closed. CDK injects the AL2023 trust-bundle path for its current PostgreSQL Lambdas; its CA rotation follows the runtime image/config release. Pools are lazy, validate idle connections, reap them after 60 seconds, recycle them after 10 minutes, and configure 2s acquisition, 5s statement, 1s lock, and 10s idle-transaction limits. A pool limit is per warm environment, never a global RDS connection ceiling. Credential refresh/replacement ownership remains #1780.
...
| Worker compact jobs/CDC fanout | `src/aura-historia-worker/src/{lib.rs,cdc.rs,jobs.rs}` plus `src/product-listing-opensearch-lambda/` for the first Lambda adapter | `docs/events/flow.md` is the contract summary, not deployment proof. |
...
`docs/events/flow.md` links the checked-in `durable-worker-runbook.md`; it is a procedure, not operational evidence. `product-listing-raw-normalization-runbook.md` remains missing. `infra/README.md` also describes API routes, IdPs, Pipes, and scheduled Fargate images which current CDK does not declare. These are documentation drift, not evidence that the resources exist.
...
| --- | --- | --- | --- | --- | --- |
| `product-listing-opensearch`: direct approved fixture source messages; future committed-event publisher remains separate | `ProductListingEventJob`, event/listing IDs; all supported route union | applied, tombstoned/deleted, version-stale | missing source or required sale FX retries; malformed job is poison | dedicated Lambda → `ProjectProductListing`; PostgreSQL + OpenSearch; external projection version; 270s visibility/45s Lambda, batch one | #1783 |
...
**Current custody declaration.** Ten Standard source/DLQ pairs are declared: source 7d, DLQ 14d, redrive 5, long poll 20s, SSE/TLS policies; prod retains queue deletion/replacement. `product-listing-opensearch` has the first SQS Lambda mapping, version target, batch size one, partial-failure response, 45s timeout, and 270s source visibility. The other nine remain polling processes with capacity one/no prefetch, heartbeat `min(30s, visibility/3)`, and jittered retry visibility 30–900s. CDK preserves unbound policies for native scopes; the external identity owner attaches them. Live deployment, mapping state, and AWS proof remain unverified.
...
| Worker queues/DLQs/messages | Declared per stage in CDK; ProductListing OpenSearch Lambda mapping is checked in; account/region not verified | In-flight jobs/DLQs; native router/fixtures and remaining native workers | #1783, #1790–#1797 | Scoped queue/Lambda preserves keys, guards, terminal-only acknowledgment, and message retention; mapping pause/drain/capture gates remain | #1806 approved operator, never runtime purge |
