...
Revalidated 2026-09-13 at clean `b89d8942194f1ca676b487a9a89febb6f502f462`, same integration branch. Current Cargo inventory: 71 packages, **15 binaries** (new excluded `bootstrap-local`); four native services, five Rust Lambdas, ten worker scopes. Running deployments remain unknown. Historical 14-bin counts below describe the original baseline.
...
- 04 foundations: cron loopback-only operations (daemon and run-once), strict non-scheduling preflight, retained execution/drain outcomes, default300/330s budgets. Crawler capture counts distinguish accepted/durable/completed; removed local completion shortcuts. Crawler pass now owns producer/collector drain; review listener owns connection tasks; discovery owns fallible producer/forwarder completion. Spider service now propagates checkpoint/count failures and joins wrappers on stop/error. Scheduler owns cooperative drain, distinguishes site retries from operational failures, and emits early failure notification. Strict server CLI/read-only preflight now checks both histories without cloud/consumer activity; exact crawler history and qualified catalog checks are proven with restricted roles. Compile-time release identity and one-shot watchdog added. Server daemon now owns both signals, private readiness/version, pool/exporter/runtime cleanup and terminal deadlines. Real idle-process tests exist; active production capture/checkpoint interruption and full host handover remain unpassed. Real PG tests prove advisory-session loss is not a transaction fence; handover requires confirmed prior-process termination.
- 05: `bootstrap-local --initialize-fresh business|crawler` and `--verify business|crawler` support explicitly isolated nonreal targets only. No real-stage initializer, incremental executor, adoption or historical backfill implementation.
...
- Revalidated default-off boundary: no workflow invokes hybrid control; unsupported controller commands fail nonzero. This is **not a repository-wide kill switch**: legacy manual `deploy.yml` can still invoke CDK. No GitHub protection state was inspected or changed.
