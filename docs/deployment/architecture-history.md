# Superseded hybrid design history

**Archive at cf65af2c64bd3930908b3890b64636817291fc3c; not normative.** Reset playbook rejects the renderer/generalized controller direction and supersedes the old iteration graph. Consult `architecture-decisions.md` for active decisions; retain only explicitly preserved runtime safety rules. Do not execute the roadmap below.
...
Owner: “no need for any real migrations as we're still in dev.” Defer incremental SQL/OpenSearch data migration, legacy adoption, historical backfills and upgrade/rollback-schema compatibility machinery. Retain existing baseline initialization and checks for fresh installations/isolated tests; no new migrator crate or incremental executor. Existing `bootstrap-local` now has reviewed connect-only fresh/verify modes for isolated nonreal targets (iteration05), separate from daemon startup. Real-stage packaging/provisioning still needs its approved platform path; this local tool does not authorize or implement live setup. This supersedes the migration-heavy portions of the iteration map below, not runtime custody, TLS, immutable artifacts, approval or durable deployment ownership. Existing databases are not implicitly disposable; no live execution/reset authorized. Draft catalog/command entries stay honestly unavailable until revised tooling exists.
...
## ADR-005 — Crawler Unix terminal process fence

Rust `std::process::exit` can block on shared stdout/exit cleanup; a second watchdog calling it can block on the same cleanup lock. Crawler's Unix runtime therefore uses one narrowly documented `libc::_exit` call at its terminal boundary. Normal exit first joins workloads, closes pools/exporter, destroys Tokio and explicitly flushes output inside independent deadlines. Only then may it select STOPPED/success. Watchdog expiry skips cleanup/output and exits nonzero; accepted work is failed/unknown, never credited durable by forced exit.

Daemon panic hook never prints/chains the default hook: atomically publish sticky failure and an absolute process fence before unwinding can block or leak payloads. Watchdog never waits on workload/timeline locks. Review failure signals before connection cleanup. Both signals and repeated failures cannot extend deadlines. This exception is confined to the crawler daemon and local-bootstrap executable boundaries, sharing `server_runtime/terminal.rs`, not a general permission for unsafe business/adapter code. Bootstrap also uses this non-core fence: `abort()` can dump credential-bearing memory and delay exit through a core collector. Non-Unix daemon and new bootstrap modes are unsupported; legacy/help fallback does not claim the Unix fence. Success or failed fencing is proved by observed process termination, not readiness state; cross-host takeover still requires confirmed termination or separately implemented fencing.
