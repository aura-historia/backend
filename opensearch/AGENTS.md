...
- Runtime crates and infra depend on these files staying aligned with actual indexed documents. ProductListing mapping source identity is only `listingSourceId` and `sourceListingId`; source presentation is hydrated from PostgreSQL. Search-filter documents permit only `listingSourceId` and `excludeListingSourceId` source filters. No ListingSource index exists.
