...
- Deploy workflow deploys split CDK stacks from one stage prefix, packages and pushes the six active Rust Lambda artifacts with locked Cargo/cargo-lambda inputs, publishes all 25 active MJML templates—10 `partnership-application`, 5 `search-filter/match`, and 10 `watchlist/product-update` (availability and price), each in `de`, `en`, `es`, `fr`, and `it`—and merges stack outputs for smoke tests. Changes under `migrations/**` trigger deployment validation.
