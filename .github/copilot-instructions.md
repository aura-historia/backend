...
- `product-lambda-materialize-opensearch`: Materialize product events to OpenSearch
...
- Build specific Lambda: `cd src/product-lambda/src/product-lambda-materialize-opensearch && cargo build --all-features`
...
- `product-lambda-materialize-opensearch` replays product event records for policy-event updates before bulk-indexing to OpenSearch.
