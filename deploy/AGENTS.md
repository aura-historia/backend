...
- `images/` — shared multistage Dockerfile, four nonroot runnable application targets; separate explicit dev fresh-initializer target; fixture-only bootstrap/Python helper; Python-only `opensearch-test-helper` and test-executable-only `opensearch-tls-witness` derived without the production build stage for the secured-engine CI witness; stock stunnel5.80 sidecar for Sequin PostgreSQL verification.
...
- `tests/smoke-opensearch.py` — independently repeated secured stock-node initialization, role/bulk denials, five nonroot Rust-client TLS pings after grants, durable fences, hybrid/PIT, TLS negatives and same-node restart. Failed witnesses retain only safe evidence. This fixture does not prove CDC/reboot acceptance. Fresh-only Query Insights disablement must not be blindly applied to existing exporter state.
