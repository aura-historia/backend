...
- New API/worker/cron source supports `OPENSEARCH_SSL_ROOT_CERT` via shared verified TLS; dev/prod now require it. Mount public `opensearch-ca.pem` read-only as described in Compose docs. **Historical image IDs below predate this change.** No image rebuild or secured-node application startup is credited by the loopback client/mount tests; build new source-matched immutable images before rollout.
