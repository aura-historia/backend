...
- Use `Sequin::worker_webhook()` in `#[aura_integration_test]` plus `get_sequin_worker_webhook_bind_addr()` when a test must verify real Sequin webhook delivery to a local worker endpoint. Its default sink sends insert, update, and delete changes for `product_events`, `search_filters`, and `search_filter_matches`. Scoped workers must seed fixtures before starting an isolated `start_sequin_for_tables` sink limited to their source table.
