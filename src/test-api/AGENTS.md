...
- Use `Sequin::worker_webhook()` after Postgres and target fixtures in `#[aura_integration_test]` when a test must verify real worker delivery. It starts one process-lived Redis/Sequin subscription for `product_events`, `search_filters`, and `search_filter_matches`; it has no per-test reset or shutdown. Start the worker at `get_sequin_worker_webhook_bind_addr()` before writing watched source rows.
