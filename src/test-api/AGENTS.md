...
- Postgres uses the cached pin in `postgres/image-ref.txt` (`--pull=never`); `AURA_TEST_POSTGRES_IMAGE` permits an intentional cached local test image. CI explicitly pulls and inspects the selected reference before Rust tests, then supplies that local image ID; the fixture itself has no automatic build/pull. It preloads `pg_ttl_index`, creates the extension, starts its worker, applies schema-only migrations once per suite process, then truncates application data between tests. Extension metadata survives teardown. `Postgres::new_per_test` replays seed SQL; optional setup scripts run before each test. Raw SQL replay does not stamp SQLx history; worker process fixtures own their actual SQLx migrations and ledger preservation.
- LocalStack and Postgres use process-id-scoped names and isolated host ports. Postgres uses private `postgres_fixture.rs`: `/usr/bin/docker --host unix:///var/run/docker.sock`, an exclusively created mode-0700 config directory, cleared child environment (only fixed PATH), and `/usr/bin/timeout` (20s plus 2s kill grace per Docker command). Ambient Docker context/host/TLS/cert/config/proxy settings cannot select another daemon. Requires the local Unix socket and Linux CLI tools; no runtime environment mutation.
- Postgres owns only full lowercase 64-hex IDs returned by successful create. Name collisions fail; no predelete, name cleanup, volume deletion, or pruning. Startup error/panic/cancellation, normal exit, and SIGINT/SIGTERM clean acquired IDs. Cleanup checks exact-ID existence, removes only that ID, then verifies absence (at most three bounded Docker commands). Failed cleanup retains authority for hook retry and reports only the owned ID, not provider output. Failed/malformed create responses grant no authority; safe create diagnostics use only fixed operation/status/category fields; never guess an orphan by name.
- Docker assigns Postgres's host port. Publication remains `0.0.0.0` for Sequin's separate-container `host.docker.internal` gateway connection, not loopback-only. Use an isolated local test host: fixture credentials are test-only. Data uses container tmpfs, not an anonymous persistent volume. LocalStack's existing topology checks stay unchanged.
- OpenSearch bootstraps its domain, pipelines, mappings, and indexes once per suite process; teardown clears only canonical documents, including `user_search_filters`. LocalStack gets a derived test-only mapping: inline synonym rules replace filesystem paths and only OpenSearch3.8-required SQ `bits` is removed; checked-in production mappings stay authoritative.
...
- Narrow Docker safety tests (fake CLI; one poisoned-env subprocess): `cargo test --locked --offline -p test-api --lib --features postgres postgres::fixture::tests -- --test-threads=1`
- Opt-in real fixture gate: `cargo test --locked --offline -p test-api --lib --features postgres postgres::fixture::tests::process_tests::should_start_postgres_and_verify_owned_cleanup_in_subprocesses -- --exact --ignored --nocapture --test-threads=1`. Four isolated children prove raw replay/pg_ttl/gateway access, collision survival, partial-start cancellation, and signal/exit cleanup. Parent verifies actual acquired IDs absent, with ID-only recovery on child failure. Needs cached image; never pulls. No LocalStack/Sequin/worker run. Do not run ignored child entries directly.
