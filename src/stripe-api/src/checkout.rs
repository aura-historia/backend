...
pub async fn handle(
    ...
    let price_info = stripe_service
        .get_price_by_lookup_key(billing_request.lookup_key())
        .await
...
mod tests {
    ...
    fn user_without_stripe_customer_id_with_currency(currency: Currency) -> User {
        let mut user = user_without_stripe_customer_id();
    ...
    #[rstest]
    #[case::pro_monthly(BillingPlan::Pro, BillingCycle::Monthly, "pro_monthly", "price_pro_m")]
    #[case::pro_yearly(BillingPlan::Pro, BillingCycle::Yearly, "pro_yearly", "price_pro_y")]
    ...
    async fn should_201_with_session_url_when_user_has_no_stripe_customer_id_for_each_combo(
        ...
        #[case] expected_lookup_key: &'static str,
        ...
        stripe_service
            ...
            .withf(move |key| key == expected_lookup_key)
            .return_once(move |_| {
                Box::pin(async move { Ok(price_info(expected_price_id, &["eur"])) })
            });
        stripe_service
        ...
        stripe_service
            ...
            .withf(move |_, customer_id, price_id, currency| {
    ...
    #[tokio::test]
    async fn should_201_forwarding_currency_when_user_has_matching_currency() {
        let mut user_service = MockUserService::default();
        user_service.expect_find_user().return_once(|_| {
            Box::pin(
                async move { Ok(user_without_stripe_customer_id_with_currency(Currency::Usd)) },
            )
        });
        user_service
            .expect_update_user()
        ...
        stripe_service.expect_create_customer().return_once(|_| {
            Box::pin(async move { Ok(StripeCustomerId::from("cus_freshly_created")) })
        });
        stripe_service
            .expect_create_checkout_session()
    ...
    #[tokio::test]
    async fn should_201_without_currency_when_user_currency_not_supported() {
        let mut user_service = MockUserService::default();
        user_service.expect_find_user().return_once(|_| {
            Box::pin(
                async move { Ok(user_without_stripe_customer_id_with_currency(Currency::Jpy)) },
            )
        });
        user_service
            .expect_update_user()
        ...
        stripe_service
            .expect_create_checkout_session()
            .withf(|_, _, _, currency| currency.is_none())
            .return_once(|_, _, _, _| {
                Box::pin(async move {
                    Ok(Url::parse("https://checkout.stripe.com/c/pay/cs_test_fallback").unwrap())
                })
            });
    ...
    #[tokio::test]
    async fn should_500_when_price_lookup_fails() {
        let user_service = MockUserService::default();
        let mut stripe_service = MockStripeService::default();
        stripe_service
            .expect_get_price_by_lookup_key()
