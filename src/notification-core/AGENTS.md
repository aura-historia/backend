...
- Domain-only crate.
- Root modules: `notification`, `presentation`, `notification_delivery`, `notification_delivery_id`, `notification_kind`. `NotificationId` lives in `common::notification_id` so Product user state can use it without a dependency cycle.
