...
- Domain-only crate.
- Root modules: `content_policy`, `description`, `listing_availability`, `listing_lifecycle`, `listing_orderability`, `product_listing`, `product_listing_auction`, `product_listing_event`, `product_listing_price`, `product_listing_id`, `product_listing_image`, `product_listing_search`, `product_listing_slug_id`, `sanitize`, `source_listing_id`, `sort_product_listing_field`, `title`.
...
- `product_listing::ProductListing` is canonical aggregate. Fields private. Rehydrate boundary public for adapter crates. Its optional listing-owned Auction facts contain an optional Auction ID, validated lot number, one-based catalogue position, and optional exact `bidding_opens`, `scheduled_closes`, and `reported_closed_at` instants. All-empty facts normalize to absence; opening after scheduled close rejects.
...
- `EnhancedSearchDescription` canonicalizes outer Unicode whitespace, rejects blank values, and caps stored text at 1000 bytes; raw-text construction is fallible. `ProductListingSearch` uses exact `AuctionId` membership filters and optional `ListingAvailabilityQuery`: each ID set ORs internally, dimensions intersect, exact availability and derived orderability sets intersect, and `include_unspecified` separately includes absent assertions.
