...
- OpenSearch reads are ordinary readers. No transaction or unit of work. Listing-owned Auction/lot facts stay local: documents carry optional resolved `auctionId` and exact-only `lotBiddingOpensAt` and `lotScheduledClosesAt`; lot number, catalogue position, and reported-close time are not indexed. The exact `auctionId` filter matches only resolved membership; omitted IDs never match, even when independent lot facts are present.
