...
- OpenSearch reads are ordinary readers. No transaction or unit of work. Listing-owned Auction/lot facts stay local: documents carry optional resolved `auctionId`, `lotLabel`, `lotPosition`, and exact-only `lotBiddingOpensAt`, `lotScheduledClosesAt`, and `lotReportedClosedAt`. The exact `auctionId` filter matches only resolved membership; omitted IDs never match, even when independent lot facts are present.
