...
- OpenSearch reads are ordinary readers. No transaction or unit of work. Listing-owned Auction/lot facts stay local: documents carry optional resolved `auctionId` and exact-only `lotBiddingOpensAt`, `lotScheduledClosesAt`, and `lotReportedClosedAt`; lot number and catalogue position are not indexed. The exact `auctionId` filter matches only resolved membership; omitted IDs never match, even when independent lot facts are present.
