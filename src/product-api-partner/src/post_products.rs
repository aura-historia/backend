...
pub async fn handle(
    ...
    {
        if let Some(failure) = command_product_service.create(command).await {
...
mod tests {
    ...
    async fn should_return_200_with_error_entries_when_some_products_fail() {
        ...
        command_service
            .expect_create()
            .return_once(move |_| Box::pin(async move { Some(expected_cmd) }));
    ...
    async fn should_pass_seller_name_raw_to_command_when_seller_name_provided() {
        ...
        let mut command_service = MockCommandProductService::default();
        command_service.expect_create().return_once(move |cmd| {
            Box::pin(async move {
                assert_eq!(
