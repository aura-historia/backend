...
- Search filter timestamps live on reader/index views, not aggregates. `RunPeriodicSearchFilterMatching` uses focused ports for run locking, candidate pages, dedupe, source reads, final revision guards, idempotent writes, and separate progress; ordinary Search Filter views do not expose progress.
