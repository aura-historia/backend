...
- Search filter timestamps live on reader/index views, not aggregates. `RunPeriodicSearchFilterMatching` uses focused ports for run locking, candidate pages, dedupe, source reads, final filter-version/activity and Product-revision guards, idempotent writes, and separate progress; ordinary Search Filter views do not expose progress. Its short final transaction locks and revalidates the candidate filter is `ACTIVE` at its selected version before any match insert or progress checkpoint.
