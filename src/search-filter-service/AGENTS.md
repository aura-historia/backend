...
- Depends on `search-filter-core`, shared `common` types, shared `application` transaction contracts, public ProductSearch field types from `geo`, `isocountry`, and `shop-core`, plus canonical `user-service` tier-entitlements contracts.
