...
#[cfg(test)]
mod tests {
    use super::event_store::SqlxProductEventStore;
    use super::repository::SqlxProductRepository;
    ...
    async fn should_insert_product_and_event_in_same_transaction() {
        ...
        match SqlxProductRepository::new(&mut tx)
            .insert(&product, event_id, "product-test")
            .await
        {
            Ok(()) => {}
        ...
        for event in product.pending_events() {
            match SqlxProductEventStore::new(&mut tx)
                .append(event, "product-test")
                .await
            {
                Ok(()) => {}
    ...
    async fn should_reject_stale_current_event_id() {
        ...
        let result = SqlxProductRepository::new(&mut tx)
            .update(&product, stale, replacement, "product-test")
            .await;
        ...
        assert!(matches!(
            result,
            Err(ProductRepositoryError::ConcurrencyConflict)
        ));
    ...
    async fn should_rollback_product_when_event_append_fails() {
        ...
        let duplicate_result = SqlxProductEventStore::new(&mut tx)
            .append(event, "product-test")
            .await;
        ...
        assert!(matches!(
            duplicate_result,
            Err(ProductEventStoreError::EventConflict)
        ));
    ...
    fn create_product(
        ...
        match Product::create(NewProduct {
            ...
            native_description: Some(Localized::new(
                Language::En,
    ...
    async fn seed_fx_rate(pool: &sqlx::PgPool, fx_rate_id: FxRateId) {
        ...
        let insert_rate = sqlx::query(
            r#"
            INSERT INTO fx_rates (fx_rate_id, captured_at, source, rates)
            VALUES ($1, now(), 'test', '{"EUR":{"USD":1100000}}')
            "#,
        )
    ...
    ) -> crate::service::ports::product_repository::LoadedProduct {
