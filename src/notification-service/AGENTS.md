...
- `presentation::NotificationPresentationPreferences` carries language, currency, and current prohibited-content consent. The notification-core image policy preserves the classification while exposing unsafe URLs only with consent; REST and email consume it without changing immutable notification snapshots.
