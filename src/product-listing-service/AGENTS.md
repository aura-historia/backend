...
- Write handlers use `application::transaction::UnitOfWork` and transaction-scoped ProductListing repository/event-appender factories. Listing availability is `Option<ListingAvailability>` and lifecycle is `ListingLifecycle`; generic writes do not capture FX valuations.
