...
- Create, update, upsert, and withdraw collect pure core event payloads, stamp them with `stamp_product_listing_events`, then persist the matching current event ID and append the stamped events in one transaction.
- Partner ProductListing create, update, upsert, and withdraw use cases require `ProductListingsWrite`, then authorize admins or linked partner users inside their listing transaction. Partner-key writes use `(shopId, shopListingId)` aggregate lookup. WooCommerce validates capability, partner authorization, shop config, and signature, then maps and persists ProductListing aggregates/events through direct transaction-bound ports in one transaction; it never calls another inbound use case. Missing or already-withdrawn provider deletes are committed no-ops.
...
- ProductListing detail, search, KNN, and watchlist result contracts use `application::personalized::Personalized<Item, ProductListingUserState>`; item views never inline `user_state`. `ProductListingDetailsReader` returns factual detail with source pricing, optional sale observation, availability, and lifecycle. `RecordProductListingSaleObservation` owns the authorized transactional FX lookup and event append; generic writes never capture FX.
