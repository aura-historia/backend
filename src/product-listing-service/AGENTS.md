...
- Partner ProductListing create, update, upsert, and withdraw use cases authorize admins or linked partner users inside their listing transaction. Partner-key writes use `(shopId, shopListingId)` aggregate lookup. Shopify and WooCommerce provider payloads remain provider-native at the boundary, then map into ProductListing commands and transact once.
...
- ProductListing detail, search, KNN, and watchlist result contracts use `application::personalized::Personalized<Item, ProductListingUserState>`; item views never inline `user_state`. `ProductListingDetailsReader` returns factual detail with source pricing, optional sale valuation, availability, and lifecycle.
