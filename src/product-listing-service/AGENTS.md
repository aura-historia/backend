...
- Write handlers use `application::transaction::UnitOfWork` and transaction-scoped ProductListing repository/event-appender factories. `CaptureProductListingRawObservation` uses its own transaction-bound raw-capture writer, recomputes a shared typed input hash, and writes only immutable raw revisions; it never mutates canonical listings or appends ProductListing events. Listing availability is `Option<ListingAvailability>` and lifecycle is `ListingLifecycle`; generic writes do not capture FX valuations.
