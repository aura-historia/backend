...
- Create, update, upsert, and withdraw collect pure core event payloads, stamp them with `stamp_product_listing_events`, then persist the matching current event ID and append the stamped events in one transaction. Upsert main price uses `PatchField`: unchanged preserves current price, set replaces it, and clear removes it.
