...
- OpenSearch-backed Product search first pages use a short repository read transaction to load the latest persisted FX snapshot; cursor pages load the cursor snapshot ID. The handler commits, then compiles one `ProductSearchReadRequest` with scaled native ranges and pinned conversion data. Similar-Product reads also load one latest persisted snapshot before their OpenSearch KNN read. Active summaries use that snapshot; sold summaries use immutable indexed sale values. Missing, invalid, read, begin, and commit failures are explicit. OpenSearch and embedding stay outside this transaction.
