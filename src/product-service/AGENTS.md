...
- OpenSearch-backed search first uses a short repository read transaction to load one latest persisted FX snapshot, commits it, then compiles one `ProductSearchReadRequest` with scaled native ranges and pinned conversion data. The reader receives this one compiled request. Missing, invalid, read, begin, and commit failures are explicit. OpenSearch and embedding stay outside this transaction.
