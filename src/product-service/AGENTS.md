...
- Write handlers use `common::transaction::UnitOfWork` and transaction-scoped repository/event-store factories. Sold Product creation and transitions capture `sold_at`, then rehydrate latest persisted FX snapshot at or before that instant through a transaction-scoped FX repository, then store immutable sale valuation; missing or invalid data rejects the write.
...
- `ProductUserStateReader` is an ordinary one-query batch read for relational state of OpenSearch result pages, including ordered unseen notification IDs. Its lookup contains only the user and Product IDs; adapters derive image safety from authoritative Product data. Product details readers return the same complete `ProductUserState` in their caller transaction; no second notification read, per-product read, or partial fallback.
...
- `GenerateWatchlistNotifications` owns Product-event-driven watchlist notifications: it reads the immutable Product event/source, including `product_events.event_time` and optional source-currency prices, locks and rechecks the current Product revision, selects recipients by current watchlist intervals, then inserts one in-app notification per eligible recipient plus requested external delivery intents in one short PostgreSQL transaction. Its successful results are `Applied`, `SuppressedForMissingSource`, and `SuppressedForStaleProductEvent`; applied work reports inserted and deduplicated counts separately.
