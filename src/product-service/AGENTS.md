...
- `NormalizeProductListingRawRevisionUseCase` drains immutable raw streams in order; it accepts payload schema `1` and raw-values schema `1`. Other stored versions fail before canonical work and stay pending for an authorized development reset; no conversion is attempted. A `System` normalizer configuration failure is retryable before transaction/completion; malformed current raw contracts write terminal `REJECTED`. Completion records the normal outcome and advances progress atomically. Direct CDC errors stay errors.
...
- Use the caller-owned PostgreSQL transaction with the ProductListing-only canonical writer. Raw writes never resolve, create, or mutate Auction facts; ordinary product updates preserve existing membership and lot state.
