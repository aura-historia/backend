...
- Depends on `product-core`, `shop-core`/`shop-service` for Shopify intake eligibility, `notification-service`, shared `common` app contracts, and product-neutral `embedding`. Product text search passes typed `EmbeddingInput::Query` directly to `EmbeddingGenerator`; embedding failure falls back to BM25.
...
- Partner Product create, update, upsert, and delete use cases authorize admins or linked partner users inside their Product transaction. Partner-key writes use `(shopId, shopsProductId)` aggregate lookup. Shopify intake resolves published partner shops through `shop-service` then delegates the authoritative product/event transaction to Product upsert.
