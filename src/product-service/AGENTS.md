...
- `GenerateWatchlistNotifications` owns Product-event-driven watchlist notifications: it reads the immutable Product event/source and active recipients, then inserts one in-app notification per recipient plus requested external delivery intents in one short PostgreSQL transaction. Its result reports inserted and deduplicated counts separately.
