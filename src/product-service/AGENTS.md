...
- Depends on `product-core`, `shop-core`/`shop-service` for Shopify intake eligibility, `notification-core`/`notification-service`, direct `application` contracts, and product-neutral `embedding`. Product text search passes semantic query text to `EmbeddingGenerator::embed_search_query`; embedding failure falls back to BM25.
