...
- Membership and ListingSource grants use idempotent join-table inserts. Membership writes report whether a row was added or already existed. A source grant belongs to `(partnership_id, listing_source_id)`; authorization requires both a user membership and that Partnership grant.
