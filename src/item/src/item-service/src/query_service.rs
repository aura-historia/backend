        ...
        fn from(err: SearchItemsError) -> Self {
            match err {
                SearchItemsError::OpenSearchError(err) => {
                    error!(error = %common::json_error!(err), "Encountered OpenSearchError while searching items.");
...
mod tests {
    ...
    #[case(
        SearchFilter {
            ...
            state_query: AnyOfQuery::from(HashSet::from_iter([ItemState::Available, ItemState::Listed])),
