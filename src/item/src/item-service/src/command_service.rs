...
impl<T: FxRate + Sync> CommandItemServiceImpl<'_, T> {
    async fn handle_create_chunk(
            ...
            .await
        {
            Ok(existing_item_keys) => {
                ...
                for batch in batches {
                    ...
                    match res {
                        ...
                        Err(err) => {
                            error!(error = %common::json_error!(err), "Failed writing entire ItemEventRecord-Batch due to SdkError.");
            ...
            Err(err) => {
                error!(error = %common::json_error!(err), "Failed entire BatchGetItem-Operation due to SdkError.");
