        ...
        fn from(err: GetItemError) -> Self {
            match err {
                ...
                GetItemError::SdkGetItemError(err) => {
                    error!(error = %common::json_error!(err), "Encountered SdkGetItemError while getting item.");
                    err.into()
                }
                GetItemError::SdkQueryError(err) => {
                    error!(error = %common::json_error!(err), "Encountered SdkQueryError while querying item and its history.");
                    err.into()
                }
            }
        }
