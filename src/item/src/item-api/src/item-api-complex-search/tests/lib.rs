use common::api::collection::CollectionData;
use fake::{Fake, Faker};
...
async fn should_200_when_no_hits() {
    ...
    let collection_data: CollectionData<GetItemData> = serde_json::from_value(json).unwrap();
    assert!(collection_data.items.is_empty());
    assert_eq!(0, collection_data.pagination.total);
}
