...
- Depends on `watchlist-core`, Product and FX read contracts, Notification read contracts, shared `common` identifiers/ports, pure `money`/`localization` values, and shared `application` transaction contracts.
