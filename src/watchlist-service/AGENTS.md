...
- List uses transaction-scoped ProductListing watchlist-details and FX snapshot readers for one PostgreSQL cursor page. The ProductListing reader joins current parent Auction presentation and returns separate listing-owned lot facts; list has no post-commit Auction lookup. ProductListing details return canonical `ProductListingUserState`, including notification state, directly. Service applies the ProductListing pricing presentation policy: one latest FX snapshot for all current valuations and one batch lookup for sale valuation snapshots. Missing or invalid FX data fails explicitly.
