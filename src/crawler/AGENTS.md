...
- Main neighbors: `application`, `large-language-model`, `listing-source-core`, `localization`, `money`, `platform-postgres`, `product-listing-core`/`product-listing-service`/`product-listing-postgres`, `shop-core`/`shop-service`/`shop-postgres`.
...
- Postgres be crawler source of truth. Main durable tables be `listing_sources`, `listing_source_domains`, `listing_source_urls`, `listing_source_product_schemas`, `listing_source_removed_page_schemas`, `crawler_reviews`, `crawler_review_pages`, `listing_availability_mapping`. `listing_sources.crawl_enabled` gates work only; disabling preserves local domains, configuration, reviews, and history.
- Main handoff be DB-backed: shop sync feeds spider; spider feeds scraper through `listing_source_urls`; scraper calls the canonical `product-listing-service` upsert use case against authoritative business Postgres with `listing_source_id` and `source_listing_id`. Crawler-local Shop IDs map to ListingSource IDs only at this handoff. Crawler creates no Party, seller, or auctioneer attribution; raw marketplace seller names are not canonical attribution.
...
- Shop-level LLM spend be budgeted through `listing_sources.llm_calls_count`.
