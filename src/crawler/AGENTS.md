...
- Cached schema scoring lives in `scraper::scraper_service::extraction::schema_candidates`. Each populated prepared logical field counts once (multiple description fragments → one, valid images → one, every populated raw-attribute key → one); `default_currency` is schema context and does not score. Stored order only breaks score ties.
