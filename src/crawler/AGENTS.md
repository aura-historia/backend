...
- Domain ownership is crawler-local. Register/list/remove only through the guarded review API: `GET`/`POST /api/listing-sources/{listingSourceId}/domains`, `DELETE /api/listing-sources/{listingSourceId}/domains/{domainId}`. Domain registration validates canonical `Domain`, is same-source idempotent, cannot transfer another source domain, and never enables crawling.
- Every URL references its owning `(listing_source_id, domain_id)` through a composite foreign key. Exact URLs are globally unique; cross-source URL conflicts fail. URL-pattern state and URL-pattern reviews are domain-scoped. Product-schema reviews remain ListingSource-scoped.
