...
- Product normalization uses `product-listing-normalization` pure APIs for candidate plausibility. Availability returns resolved, no-assertion, or unsupported outcomes. `listing_source_urls.crawler_disposition` is `ACTIVE`, `DORMANT_SOLD`, or `DORMANT_REMOVED`; scrape and schema-seed selection accept only `ACTIVE`. Only resolved `SoldOut` becomes `DORMANT_SOLD`, after durable raw UPSERT capture. Verified removal becomes `DORMANT_REMOVED` only after durable raw DELETE capture. Unsupported, missing, ambiguous, invalid, and non-sold availability remain active; transient failures never change disposition. Spider rediscovery preserves disposition.
...
- Initial multi-page generation accepts product schema responses only. Fresh single-page generation accepts product, removed, and not-product classifications. Removed needs verified selector-bound text or regex evidence, stores ListingSource-scoped `listing_source_removed_page_schemas`, captures a raw DELETE, then marks the URL `DORMANT_REMOVED` only after capture commits. Not-product needs verified reason and only changes that URL class to `other`; never update a domain URL pattern from one page.
...
- Demo raw-capture file snapshots are display-only, never replay input; they retain source payload, generic raw values, normalization context, and provenance.
