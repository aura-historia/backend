...
- Scraper applies all cached schemas to one parsed page, validates a candidate-local copy including images, ranks by usable completeness, then normalizes richest to least rich. Raw capture keeps the untouched extraction. Generic crawler schemas never infer auction identity or lot timing. A source-specific, fixture-backed extractor may map reviewed selector evidence plus its exact tested URL namespace to the current raw Auction patch; Lot-tissimo alone currently does this for its `catalogue-id-…` lot URLs, catalogue name, lot label, and nonblank date-only `lotStartDate` as lot opening and `lotEndDate` as lot close. It preserves participation when no source key is present, but only a source key can resolve Auction membership. It never treats the `Live` label as a lot close or an opening price as a listing price. Candidate-data failures reject only that schema; external/system failures abort and never trigger fresh generation. Fresh generation starts only after cached candidates exhaust, and cached schemas are never modified or generation inputs.
