...
- Product normalization completes deterministic preparation before state mapping. A deterministic candidate-data failure makes no state-mapping DB/LLM call and consumes zero state-mapping LLM budget. Iteration 4 maps listed/unknown to no availability, available to `Available`, reserved to `Reserved`, sold to `SoldOut`, and removed to no availability; removed-page handling stays crawler-local.
