...
- Product normalization uses `product-listing-normalization` pure APIs. Availability returns resolved, no-assertion, or unsupported outcomes. Unsupported availability preserves canonical state; only verified removal evidence changes crawler-local presence.
