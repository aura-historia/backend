...
- Production export: `verify_business_schema(&PgPool) -> Result<(), PostgresSchemaError>`. API/worker/cron call once before accepting work; wiring belongs to runtime owners. No config/TLS changes.
...
- Requires installed `pg_trgm`, `unaccent`, `pg_ttl_index` in `public` and all 35 baseline business tables as persistent ordinary/partitioned tables. Ledger must be a persistent ordinary table without RLS; views/RLS cannot hide history. No business rows read.
