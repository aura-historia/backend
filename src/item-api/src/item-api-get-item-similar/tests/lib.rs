...
async fn should_202_when_similar_items_have_not_been_computed_for_anon() {
    ...
    let ddb_insert_res = item_dynamodb_repository
        .put_item_records([item_record.clone()].into())
        .await
        .unwrap();
    assert!(
        ddb_insert_res
...
async fn should_200_when_similar_items_have_been_computed_for_anon() {
    ...
    let mut item_document: ItemDocument = item_record.clone().into();
    ...
    assert!(actual.iter().all(|actual| {
        item_documents
            .iter()
            .any(|expected| expected.item_id == actual.item.item_id)
    }));
    assert!(
        actual
            .iter()
...
async fn should_200_and_personalize_when_similar_items_have_been_computed_for_authenticated() {
    ...
    let ddb_insert_res = item_dynamodb_repository
        .put_item_records(
            [vec![item_record.clone()], item_records.clone()]
                .concat()
                .try_into()
                .unwrap(),
        )
    ...
    for item_record in item_records.iter() {
        let _ = watchlist_service
            .create_watchlist_item(&user_id, &item_record.shop_id, &item_record.shops_item_id)
            .await
            .unwrap();
    }
