...
pub struct SqlxProductWatchlistDetailsReaderFactory;

struct SqlxProductWatchlistDetailsReader<'tx> {
...
impl SqlxProductWatchlistDetailsReaderFactory {
    pub fn new() -> Self {
        Self
    }
}
...
impl ProductWatchlistDetailsReader for SqlxProductWatchlistDetailsReader<'_> {
    ...
    > {
        ...
        let search_after = has_more
            .then(|| {
                rows.last().map(|row| ProductWatchlistDetailsCursor {
        ...
        let products = rows
            ...
            .map_err(|_| ProductWatchlistDetailsReadError::InvalidReadModel)?;
