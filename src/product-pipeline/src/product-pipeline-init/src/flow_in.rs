...
use product_pipeline_common::flow_in::{FlowInResult, MessageRef, PipeFlowIn};
use std::collections::HashMap;
use tracing::error;

#[derive(Debug, Clone)]
pub struct EventBridgeSqsDynamoDbStreamProductEventRecordPipeFlowInImpl<'a> {
    sqs: &'a Client,
    source_queue: String,
}

impl<'a> EventBridgeSqsDynamoDbStreamProductEventRecordPipeFlowInImpl<'a> {
    pub fn new(
        sqs: &'a Client,
        source_queue: impl Into<String>,
    ) -> EventBridgeSqsDynamoDbStreamProductEventRecordPipeFlowInImpl<'a> {
        Self {
            sqs,
...
impl<'a> PipeFlowIn<ProductEventRecord>
    for EventBridgeSqsDynamoDbStreamProductEventRecordPipeFlowInImpl<'a>
{
    async fn flow_in(
        &self,
        batch_in_count: u16,
        visibility_timeout: u16,
    ) -> FlowInResult<ProductEventRecord> {
        ...
        for msg in messages {
            ...
            if let Some(event_record) = extracted {
                ...
                data.insert(message_ref, event_record);
            }
        }
