...
- ProductListing search JSON stores an optional `availability_query` object with canonical exact `ListingAvailability` and derived `ListingOrderability` codes plus `include_unspecified`; state and lifecycle query fields are absent.
