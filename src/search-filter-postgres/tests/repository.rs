...
async fn seed_product(pool: &sqlx::PgPool, source_listing_id: &str) -> ProductListingId {
    ...
    sqlx::query("INSERT INTO product_listings (product_listing_id, product_listing_title_slug_id, current_event_id, content_source_event_id, listing_source_id, source_listing_id, availability, lifecycle, url) VALUES ($1, $2, $3, $3, $4, $5, NULL, 'ACTIVE', 'https://example.com/product')")
...
async fn seed_product_event(pool: &sqlx::PgPool, product_listing_id: ProductListingId) -> EventId {
    ...
    sqlx::query("UPDATE product_listings SET current_event_id = $1, version = version + 1, projection_version = projection_version + 1, updated = now() WHERE product_listing_id = $2")
        .bind(uuid::Uuid::from(event_id))
        .bind(uuid::Uuid::from(product_listing_id))
        .execute(pool)
        .await
