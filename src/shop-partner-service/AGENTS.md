...
- Depends on `shop-partner-core`, `shop-core`, `shop-service`, `user-core`, `user-service`, `notification-core`, `notification-service`, shared `common` identifiers, pure `money`/`localization` values, shared `application` transaction contracts, and shared `geo::{Geocoder, GeocodingError}`.
