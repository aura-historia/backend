...
- OAuth client metadata updates are admin-only: user and delegated-user principals require the persisted `ADMIN` role, while delegated callers also require `access-tokens:write`; update responses never expose or rotate the client secret.
