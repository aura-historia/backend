...
pub struct SqlxProductListingWatchlistDetailsReaderFactory;

struct SqlxProductListingWatchlistDetailsReader<'tx> {
...
impl ProductListingWatchlistDetailsReaderFactory<SqlxTransaction>
    for SqlxProductListingWatchlistDetailsReaderFactory
{
    fn in_transaction<'tx>(
        &'tx self,
        tx: &'tx mut SqlxTransaction,
    ) -> impl ProductListingWatchlistDetailsReader + 'tx {
        SqlxProductListingWatchlistDetailsReader {
...
impl ProductListingWatchlistDetailsReader for SqlxProductListingWatchlistDetailsReader<'_> {
    ...
    > {
        let page_size = usize::try_from(request.cursor.size)
            .map_err(|_| ProductListingWatchlistDetailsReadError::QueryFailed)?;
    ...
    ranked_matches AS (
        SELECT
            ...
            ROW_NUMBER() OVER (
                PARTITION BY
                    matched.user_id,
                    date_trunc('month', matched.created AT TIME ZONE 'UTC')
                ORDER BY
                    matched.created ASC,
