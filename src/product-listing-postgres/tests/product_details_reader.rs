...
use product_listing_service::use_cases::queries::get_product::ProductLookup;
...
async fn should_reject_product_personalization_when_authenticated_user_is_missing() {
    ...
    assert!(matches!(
        result,
        Err(
            product_listing_service::ports::ProductDetailsReadError::ProductDetailsReadModelInvalid
        )
    ));
}
...
async fn find_details_result(
    ...
    product_listing_service::ports::ProductDetailsReadError,
> {
    let unit_of_work = SqlxUnitOfWork::new(pool.clone());
    let details = SqlxProductDetailsReaderFactory::new();
    let mut tx = begin(&unit_of_work).await;
