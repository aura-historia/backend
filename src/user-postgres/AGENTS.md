...
- `SqlxUserTierEntitlementsFactory` locks `users` with `FOR UPDATE` and reconciles search-filter and watchlist plan state with legacy newest-first semantics in that same transaction. Changed watchlist rows increment their storage version.
