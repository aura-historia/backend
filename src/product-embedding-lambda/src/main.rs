use aura_historia_worker::{VERTEX_AI_LOCATION_ENV, VERTEX_AI_PROJECT_ID_ENV};
...
use product_embedding_lambda::{
    compose_product_embedding_use_case, handler_with_invocation_budget, invocation_budget,
    retain_all_records,
};
...
const COMPONENT: &str = "product-embedding-lambda";
                ...
                handle_invocation(event, move || async move {
                    ...
                    use_cases
                        .get_or_try_build(credentials.version_id(), || async {
                            ...
                            let generator = vertex.embedding_generator()?;
                            Ok::<_, Error>(compose_product_embedding_use_case(pool, generator))
                        })
                        .await
                })
                .await
...
type ProductEmbeddingUseCase = Arc<dyn EmbedProductListingEventUseCase>;
type ProductEmbeddingUseCaseLease = VersionedCompositionLease<ProductEmbeddingUseCase>;
...
where
    F: FnOnce() -> Fut,
    Fut: Future<Output = Result<ProductEmbeddingUseCaseLease, Error>>,
{
    ...
    let Some(use_case) = complete_before_invocation_deadline(&budget, setup()).await else {
        warn!(
            ...
            "Product embedding setup retained every record for SQS retry or redrive"
        );
...
impl VertexConfig {
    ...
    fn embedding_generator(&self) -> Result<VertexAiEmbeddingGenerator, Error> {
        // AccessTokenCredentials refreshes expired access tokens on each provider request, which
        // keeps a warm Lambda safe after idle periods without storing tokens ourselves.
        ...
        Ok(VertexAiEmbeddingGenerator::new(
            VertexAiEmbeddingConfig::new(self.project_id.clone(), self.location.clone()),
            credentials,
        ))
    }
}
