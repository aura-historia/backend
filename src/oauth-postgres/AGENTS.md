...
- Authorization and third-party codes persist as text credentials and consume with one `DELETE ... RETURNING` query. Deleting an OAuth client cascades its authorization codes, OAuth-issued access tokens, and their third-party exchange codes in PostgreSQL.
- `cleanup_expired_credentials_and_provider_receipts` is physical maintenance only. It rejects NULL and out-of-range batches, deletes at most the requested direct rows per target with `SKIP LOCKED`, and deletes an expired access token only after no exchange-code dependents remain; returned counts are direct per-table deletes, never cascade estimates.
