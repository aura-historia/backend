...
/// enriched with UTM params during a DynamoDB → domain mapping still compares
/// equal to its un-enriched counterpart coming from an inbound command.
...
mod tests {
    ...
    fn should_append_utm_params_when_url_has_no_query_params() {
        ...
        assert_eq!(
            result.as_str(),
            "https://example.com/shop?utm_source=aura_historia&utm_medium=referral"
        );
    }

    #[test]
    fn should_append_utm_params_when_url_already_has_query_params() {
        let url = Url::parse("https://example.com/shop?ref=homepage").unwrap();
        let result = append_utm_params(url);
        assert_eq!(
            result.as_str(),
            "https://example.com/shop?ref=homepage&utm_source=aura_historia&utm_medium=referral"
        );
    }
    ...
    fn should_not_duplicate_utm_params_when_url_already_has_utm() {
        let url =
            Url::parse("https://example.com/shop?utm_source=aura_historia&utm_medium=referral")
                .unwrap();
        let result = append_utm_params(url);
        assert_eq!(
            result.as_str(),
    ...
    fn should_preserve_non_utm_params_when_stripping_utm_params() {
        ...
        .unwrap();
        let result = strip_utm_params(url);
        assert_eq!(result.as_str(), "https://example.com/shop?ref=homepage");
    }
