...
- `search_filter_percolator.rs` maps domain/enrichment `ProductListingEvent` jobs to the inbound matching command and invokes the service. The service rereads committed ProductListing state with its exact persisted sale FX snapshot, skips stale triggers whose event ID no longer matches current state, then locks/rechecks the ProductListing revision in the final Postgres match transaction before canonical match persistence. It evaluates enhanced filters outside Postgres. No legacy evaluator is linked.
- `search_filter_match_notifications.rs` maps persisted-match insert jobs to the inbound notification command and invokes the service. Match jobs and source reads use `(user_id, user_search_filter_id, product_listing_id, origin_event_id)`; stale or superseded rows suppress without notification. The service creates one idempotent PostgreSQL SearchFilter notification per matching filter after source validation and snapshots the assessment for the locked current ProductListing event. Worker logs distinguish inserted notifications from semantic duplicates.
- `watchlist_notifications.rs` consumes price/availability ProductListing event jobs, rereads committed Postgres source, locks and rechecks the ProductListing revision through notification commit, selects recipients using `product_listing_events.event_time` and current watchlist intervals, then creates idempotent PostgreSQL notification records. Its worker acknowledges and logs `Applied`, duplicate, missing-source suppression, and stale-event suppression separately; only actual failures retry or enter the in-memory DLQ.
...
- `product_embedding.rs` consumes `PRODUCT_LISTING_CREATED` ProductListing event jobs, rereads committed current ProductListing state, invokes the neutral Vertex embedding capability before a short transaction, and atomically persists the vector plus `ENRICHMENT_EMBEDDED`. Stale and duplicate jobs are explicit no-ops.
- `product_content_assessment.rs` consumes only `PRODUCT_LISTING_CREATED` ProductListing event jobs, reads the listing text source, and stores a content-source-revision guarded assessment. Price, availability, URL, image, lifecycle, embedding, and translation events do not route there.
- `product_translation.rs` consumes `ENRICHMENT_EMBEDDED` ProductListing event jobs, uses `localization::Language`, rereads committed current ProductListing source, invokes the configured neutral Vertex LLM title translator, and atomically persists canonical Postgres translations plus one translated-titles enrichment event. Stale and duplicate jobs are explicit no-ops.
...
- Runtime scope comes from required `AURA_HISTORIA_WORKER_SCOPE`: `search-filter-projection` accepts only `search_filters`; `search-filter-percolator` accepts only domain/enrichment `product_listing_events` inserts; `search-filter-match-notification` accepts only `search_filter_matches` inserts; `watchlist-notification` accepts only canonical price/availability `product_listing_events` inserts; `notification-delivery` accepts only `notification_deliveries` inserts; `product-embedding` accepts only `PRODUCT_LISTING_CREATED` `product_listing_events` inserts; `product-content-assessment` accepts only `PRODUCT_LISTING_CREATED` `product_listing_events` inserts; `product-translation` accepts only `ENRICHMENT_EMBEDDED` `product_listing_events` inserts; `product-listing-opensearch` accepts only `product_listing_events` inserts. Only `STAGE=ephemeral`, `local`, or `test` may default to projection. Other tables fail delivery rather than filling unconsumed queues. Configure each Sequin subscription accordingly.
