...
- `watchlist_notifications.rs` consumes price/availability ProductListing event jobs, rereads committed Postgres source, locks and rechecks the ProductListing current event through notification commit, selects recipients using `product_listing_events.event_time` and current watchlist intervals, then creates idempotent PostgreSQL notification records. Its worker acknowledges and logs `Applied`, duplicate, missing-source suppression, and stale-event suppression separately; only actual failures retry or enter the in-memory DLQ.
