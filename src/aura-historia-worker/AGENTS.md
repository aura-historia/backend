...
- `notification_delivery.rs` consumes notification-delivery insert jobs and invokes the canonical delivery use case. The use case claims the durable PostgreSQL delivery lease, sends through SES, then records a delivered, retryable, or permanent outcome with that lease.
