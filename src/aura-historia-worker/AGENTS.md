...
- `product_listing_opensearch.rs` consumes `ProductOpenSearch` jobs, rereads the committed current Product source by stable event/Product IDs, loads an immutable sale snapshot only when a main source price needs sale-time conversion, then writes or deletes the canonical rebuildable Product document with OpenSearch external version protection from `products.projection_version`.
