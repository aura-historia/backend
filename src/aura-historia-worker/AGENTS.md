...
- `product_translation.rs` consumes `ENRICHMENT_EMBEDDED` ProductListing event jobs, uses `localization::Language`, rereads the committed current ProductListing source, invokes the configured neutral Vertex LLM title translator, and atomically persists canonical Postgres translations plus one translated-titles enrichment event. Stale and duplicate jobs are explicit no-ops.
