...
- `cdc.rs` normalizes Sequin webhook JSON to domain jobs and fans out only after strict ProductListing v1 header and payload-shape validation. ProductListing jobs carry typed event and ProductListing IDs only; raw event strings do not cross the router boundary.
