...
- `search-filter-projection`, `search-filter-percolator`, `product-listing-opensearch` alone read OpenSearch inputs. They require `OPENSEARCH_ENDPOINT_URL`; `dev`/`prod` also require `OPENSEARCH_USERNAME`, `OPENSEARCH_PASSWORD`, HTTPS and `OPENSEARCH_SSL_ROOT_CERT`. Only explicit `local`/`test`/`ephemeral` retain optional CA and HTTP; any supplied CA with HTTP fails. Shared `platform-opensearch::tls` rejects unknown stages, URL userinfo/query/fragment, empty/non-Unicode/unreadable/nonregular/oversized/invalid CA input. CA is a nonempty PEM bundle <=1 MiB, frozen once during scoped config parsing; replacement/removal affects only next config load. Restart to rotate trust.
- `WorkerOpenSearchConfig::tls()` exists for the actual binary SDK and preflight consumers; both use the same frozen trust. Built-in roots remain trusted (not exclusive CA pinning); certificate/hostname verification stays on. SDK proxy disabled; direct preflight disables proxies/redirects and keeps 3s connect/5s total deadlines, 64KiB cap and read-only root GET. SDK redirect behavior remains pinned SDK behavior; no new SDK timeout policy. Config TLS causes stay typed/redacted; SDK build failure drops unsafe provider details.
- Private `opensearch_config_tests` reuse public test-only `src/postgres-test-ca.crt`, never deployment trust. Cover all scoped requirements/local stages, unrelated-scope isolation, malformed/non-Unicode input, URL/stage rejection, error-chain redaction, bounded CA and frozen SDK/direct builders. The ignored secured-image witness uses `WorkerStartupConfig::from_env` and the private SDK constructor once for each of the three OpenSearch scopes, with generated internal dev TLS and scoped Basic auth. Non-Unicode env tests use isolated bounded subprocesses. No visibility widened solely for tests.
...
- Worker uses workspace `platform-opensearch`, `aws-sdk-sqs`, `axum`, `strum`, `strum_macros`, plus pinned `hyper` (`server,http1`) and `hyper-util` (`tokio,service`). Update manifests/lockfile and black-box process/Sequin/SQS/DLQ acceptance together.
...
- `--bin aura-historia-worker opensearch_tls_tests::` runs private parent-owned loopback TLS exchanges through actual SDK/preflight constructors: dev/prod frozen CA, specific certificate-alert negatives and version checks. Fixture source is reused only by cron tests; no production/test API dependency added. No queue/domain/cloud/paid calls; children reaped, threads joined, owned temp files removed.
...
- CA integration evidence and rollback limits: `docs/deployment/implementation-status.md`.
