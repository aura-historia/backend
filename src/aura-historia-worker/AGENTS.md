...
- `notification_delivery.rs` consumes notification-delivery insert jobs and invokes the canonical delivery use case. The use case claims the durable PostgreSQL delivery lease, dispatches by persisted channel, then records a delivered, retryable, or permanent outcome with that lease. EMAIL currently uses S3 templates and SES.
