...
- `watchlist_notifications.rs` consumes price/state Product event jobs, rereads committed Postgres source plus active watchlist recipients, then creates idempotent PostgreSQL notification records. Its worker log reports inserted and deduplicated counts separately.
