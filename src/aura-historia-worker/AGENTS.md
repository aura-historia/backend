...
- `watchlist_notifications.rs` consumes price/state Product event jobs, rereads committed Postgres source, locks and rechecks the Product revision through notification commit, selects recipients using `product_listing_events.event_time` and current watchlist intervals, then creates idempotent PostgreSQL notification records. Its worker acknowledges and logs `Applied`, duplicate, missing-source suppression, and stale-event suppression separately; only actual failures retry or enter the in-memory DLQ.
...
- `product_embedding.rs` consumes `DOMAIN_CREATED` ProductListing event jobs, rereads committed current ProductListing state, invokes the neutral Vertex embedding capability before a short transaction, and atomically persists the vector plus `ENRICHMENT_EMBEDDED`. Stale and duplicate jobs are explicit no-ops.
...
- Runtime scope comes from required `AURA_HISTORIA_WORKER_SCOPE`: `search-filter-projection` accepts only `search_filters`; `search-filter-percolator` accepts only domain/enrichment `product_listing_events` inserts; `search-filter-match-notification` accepts only `search_filter_matches` inserts; `watchlist-notification` accepts only canonical price/state `product_listing_events` inserts; `notification-delivery` accepts only `notification_deliveries` inserts; `product-embedding` accepts only `DOMAIN_CREATED` `product_listing_events` inserts; `product-translation` accepts only `ENRICHMENT_EMBEDDED` `product_listing_events` inserts; `product-listing-opensearch` accepts only `product_listing_events` inserts. Only `STAGE=ephemeral`, `local`, or `test` may default to projection. Other tables fail delivery rather than filling unconsumed queues. Configure each Sequin subscription accordingly.
