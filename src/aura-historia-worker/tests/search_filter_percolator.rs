...
async fn ignored_product_events_flow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = async {
        ...
        let (product_id, policy_event) = create_product_with_event(
            &worker.pool,
            &product_listing_query,
            "POLICY_ACCEPTED",
            "POLICY",
...
async fn create_product_with_event(
    ...
    sqlx::query("INSERT INTO shops (shop_id, shop_slug_id, name, shop_type, partner_status, shop_domains) VALUES ($1, $2, $3, 'COMMERCIAL_DEALER', 'SCRAPED', '{}')")
        ...
        .execute(&mut *tx)
        .await?;
    sqlx::query("INSERT INTO products (product_id, product_slug_id, event_id, shop_id, seller_id, shop_listing_id, title_text, title_language, description_text, description_language, state, lifecycle, url, product_images) VALUES ($1, $2, $3, $4, $4, $5, $6, 'en', 'Worker percolator description', 'en', 'LISTED', 'ACTIVE', 'https://example.test/product', '[]')")
        .bind(product_uuid)
