...
- Keeps OpenSearch documents and mappings private; structural documents use canonical semantic leaves through local codecs, and public percolation helpers expose no document type. Product language keeps the historical uppercase OpenSearch vocabulary through its local codec. Persistent Product pricing stays native or immutable sale-time: a sold no-main-price document has complete sale metadata but no `salePrices`; temporary percolation prices use the closed-world `priceByCurrency` shape.
