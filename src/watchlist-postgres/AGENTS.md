...
- Repository writes return storage-neutral persisted watchlist state with a private storage version. Updates and deletes enforce optimistic concurrency. Insert and update maintain `active_since` and `notifications_enabled_since` as current-interval metadata.
