  ...
  "logs": {
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "/var/log/product-enrichment.log",
            "log_group_name": "/aws/ec2/product-enrichment-${STAGE_NAME}",
...
LOG_PATH="/var/log/product-enrichment.log"
sudo touch "$LOG_PATH"
sudo chmod 666 "$LOG_PATH"

# Download and install binary
sudo aws s3 cp "s3://${ARTIFACT_BUCKET}/product-enrichment-${STAGE_NAME}-${COMMIT_SHA}" /usr/local/bin/product-enrichment --region eu-central-1
sudo chmod +x /usr/local/bin/product-enrichment
...
# Run binary
echo "[$(date)] Running product-enrichment..." | tee -a "$LOG_PATH"
/usr/local/bin/product-enrichment >> "$LOG_PATH" 2>&1
EXIT_CODE=$?
