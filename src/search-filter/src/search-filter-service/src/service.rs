        ...
        fn from(err: SearchFilterError) -> Self {
            match err {
                ...
                SearchFilterError::SdkGetItemError(err) => {
                    error!(error = %common::json_error!(err), "Encountered SdkGetItemError while getting search-filter.");
                    err.into()
                }
                SearchFilterError::SdkQueryError(err) => {
                    error!(error = %common::json_error!(err), "Encountered SdkQueryError while querying search-filters.");
                    err.into()
                }
                SearchFilterError::SdkPutItemError(err) => {
                    error!(error = %common::json_error!(err), "Encountered SdkPutItemError while saving search-filter.");
                    err.into()
                }
                SearchFilterError::SdkDeleteItemError(err) => {
                    error!(error = %common::json_error!(err), "Encountered SdkDeleteItemError while deleting search-filter.");
                    err.into()
                }
            }
        }
