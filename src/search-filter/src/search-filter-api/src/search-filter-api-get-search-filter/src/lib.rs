...
use common::{
    api::{
        ...
        error::ApiError,
        error_code::{BAD_PARAMETER, INTERNAL_SERVER_ERROR, INVALID_UUID},
    },
...
// /api/v1/searchFilters/{searchFilterId}
pub async fn handle(
    event: LambdaEvent<ApiGatewayV2httpRequest>,
    service: &impl SearchFilterService,
    ...
    let search_filter_id = event
        ...
        .ok_or_else(|| ApiError::bad_request(BAD_PARAMETER).with_path_field("searchFilterId"))?
        .map_err(|err| {
...
mod tests {
    ...
    async fn should_400_when_path_param_search_filter_id_missing() {
        ...
        assert_eq!("BAD_PARAMETER_VALUE", json["error"]);
    }
