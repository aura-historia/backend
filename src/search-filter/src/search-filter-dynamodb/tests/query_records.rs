...
use search_filter_dynamodb::search_filter_record::mk_pk;
use test_api::*;
...
async fn should_return_no_records_when_partition_empty(#[case] scan_index_forward: bool) {
    let actual = get_repository()
        .await
        .query_search_filter_records(&Faker.fake(), scan_index_forward)
        .await
        .unwrap();
...
async fn should_return_records_only_for_target_user(#[case] scan_index_forward: bool) {
    ...
    let expected = Faker.fake::<SearchFilterRecord>();
    let _ = repository
        .put_search_filter_record(expected.clone())
        .await
        .unwrap();
    let other = Faker.fake::<SearchFilterRecord>();
    let _ = repository
        .put_search_filter_record(other.clone())
        .await
        .unwrap();

    let actual = repository
        .query_search_filter_records(&expected.user_id, scan_index_forward)
        .await
        .unwrap();
...
async fn should_sort_by_time_aka_uuidv7_asc_when_scan_index_forward_true() {
    ...
    let mut records = fake::vec![SearchFilterRecord; 100];
    ...
    let actual = repository
        .query_search_filter_records(&user_id, true)
        .await
        .unwrap();
...
async fn should_sort_by_time_aka_uuidv7_desc_when_scan_index_forward_false() {
    ...
    let actual = repository
        .query_search_filter_records(&user_id, false)
        .await
        .unwrap();
