...
- `partner_products/` owns synchronous partner Product batch create, update, upsert, and delete controllers at `POST`, `PATCH`, `PUT`, and `DELETE /api/v1/shops/{shopId}/products`. Each entry calls one service use case; partial batches return `200` with failed `{ shopId, shopsProductId }` items.
