...
- `users/` owns account, admin user (`GET /api/v1/admin/users` collection search and `GET /api/v1/admin/users/{user_id}` detail; legacy `/api/v1/users/{user_id}` PATCH/DELETE item operations), and access-token REST controllers.
