...
- `parties/` owns admin Party collection search/create and item detail/update controllers at `GET`/`POST /api/v1/admin/parties` and `GET`/`PATCH /api/v1/admin/parties/{partyId}`. Updates expose only Party name/contact patch behavior and preserve the immutable slug.
