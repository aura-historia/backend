...
- `oauth/` owns OAuth REST controllers for client registration, the admin-only bounded client collection at `GET /api/v1/admin/oauth-clients`, authorization code, token, revoke, and introspection flows. Client administration requires the persisted `ADMIN` role for user principals; delegated collection reads also require `access-tokens:read`. Service use cases enforce delegated `access-tokens:read`/`write` capabilities, and authorization requests cannot exceed delegated caller scopes. The admin collection uses fixed created/client UUID cursor ordering, never returns client secrets, and always uses `Cache-Control: no-store`; OAuth protocol routes and existing client detail/write routes remain under `/api/v1/oauth`.
