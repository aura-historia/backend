...
- `partnerships/` owns the admin-only bounded Partnership collection at `GET /api/v1/admin/partnerships`; it uses deterministic cursor pagination, exact Party/member/ListingSource filters, `Cache-Control: no-store`, and safe Party/count summaries without member identities, contact data, provider credentials, or other secrets.
...
- `partnerships/` — admin partnership collection REST controller.
