...
- `users/` owns account, admin user (`GET /api/v1/admin/users` collection search, `GET /api/v1/admin/users/{user_id}` detail, and `PATCH /api/v1/admin/users/{user_id}` mutation; legacy `/api/v1/users/{user_id}` only keeps DELETE), and access-token REST controllers.
