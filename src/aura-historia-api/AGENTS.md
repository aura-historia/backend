...
- `partnerships/` owns the admin-only bounded Partnership collection/detail routes at `GET /api/v1/admin/partnerships` and `GET /api/v1/admin/partnerships/{partnershipId}`; collection uses deterministic cursor pagination and exact Party/member/ListingSource filters, while detail returns bounded current references and complete counts. Both use `Cache-Control: no-store` and safe Party data without contact data, provider credentials, or other secrets.
