...
- `partnerships/` owns the admin-only bounded Partnership collection/detail routes at `GET /api/v1/admin/partnerships` and `GET /api/v1/admin/partnerships/{partnershipId}`, plus idempotent membership grants at `PUT /api/v1/admin/partnerships/{partnershipId}/members/{userId}`; collection uses deterministic cursor pagination and exact Party/member/ListingSource filters, detail returns bounded current references and complete counts, and grants return `204 No Content` for both insert and existing-member no-op. All use `Cache-Control: no-store` and safe Party data without contact data, provider credentials, or other secrets.
