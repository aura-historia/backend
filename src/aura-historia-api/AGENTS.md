...
- `partnerships/` owns the admin-only bounded Partnership collection/detail routes at `GET /api/v1/admin/partnerships` and `GET /api/v1/admin/partnerships/{partnershipId}`, plus idempotent membership grants/revocations at `PUT`/`DELETE /api/v1/admin/partnerships/{partnershipId}/members/{userId}` and ListingSource grants at `PUT /api/v1/admin/partnerships/{partnershipId}/listing-source-grants/{listingSourceId}`; collection uses deterministic cursor pagination and exact Party/member/ListingSource filters, detail returns bounded current references and complete counts, and mutations return `204 No Content` for both changed and no-op outcomes. ListingSource grants require the Partnership and ListingSource to share a Party. All use `Cache-Control: no-store` and safe Party data without contact data, provider credentials, or other secrets.
