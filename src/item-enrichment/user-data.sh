  ...
  "logs": {
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "/var/log/item-enrichment.log",
            "log_group_name": "/aws/ec2/item-enrichment-${STAGE_NAME}",
...
LOG_PATH="/var/log/item-enrichment.log"
sudo touch "$LOG_PATH"
sudo chmod 666 "$LOG_PATH"

# Download and install binary
sudo aws s3 cp "s3://${ARTIFACT_BUCKET}/item-enrichment-${STAGE_NAME}-${COMMIT_SHA}" /usr/local/bin/item-enrichment --region eu-central-1
sudo chmod +x /usr/local/bin/item-enrichment
...
# Run binary
echo "[$(date)] Running item-enrichment..." | tee -a "$LOG_PATH"
/usr/local/bin/item-enrichment >> "$LOG_PATH" 2>&1
EXIT_CODE=$?
