...
// ---------------------------------------------------------------------------
// Product matcher service: match created but no notification when quota exceeded
// Verifies that when a user has exceeded their monthly quota, the service still
// produces a SearchFilterProductMatch but does not produce a notification command.
// ---------------------------------------------------------------------------
...
async fn should_create_match_but_not_notification_when_user_exceeded_search_filter_match_quota() {
    ...
    use search_filter_lambda_percolate_product::service::{
        ProductMatcherService, ProductMatcherServiceImpl,
    };
    use user::core::tier::UserTier;
    use user::dynamodb::repository::UserDynamoDbRepositoryImpl;
    use user::service::user_service::{UserService, UserServiceImpl};
    ...
    // Create a Free tier user (quota of 10)
    let user_id = UserId::new();
    let user = user_service
        .create_user(user::service::command::CreateUserCommand {
            id: user_id,
            email: "quota-match-test@example.com".parse().unwrap(),
        })
        .await
        .unwrap();
    assert_eq!(user.tier, UserTier::Free);
    ...
    // Fill the user's quota with existing match records
    for i in 0..free_quota {
        let shop_id = common::shop_id::ShopId::new();
        let shops_product_id = common::shops_product_id::ShopsProductId::new();
        let created = now - time::Duration::hours(i as i64);
    ...
    let search_filter_service =
        UserSearchFilterServiceImpl::new(&search_filter_repo, &user_service);
    let match_count = search_filter_service
        .count_user_search_filter_matches_for_this_month(&user_id)
        .await
        .unwrap();
    assert_eq!(match_count as u32, free_quota);

    // Build a mock UserSearchFilterService that returns a matching filter
    // and delegates the quota count to the real DynamoDB-backed service
    let new_filter_id = UserSearchFilterId::new();
    let product: Product = Faker.fake();
    let product_clone = product.clone();
    let summary = UserSearchFilterSummary {
        user_id,
        user_search_filter_id: new_filter_id,
        name: UserSearchFilterName::from("Quota Test Filter"),
        enhanced_search_description: None,
        notifications: true,
        created: now,
        updated: now,
    ...
    // Create mock services for percolation and product retrieval
    let mut mock_filter_service =
        search_filter::service::user_search_filter_service::MockUserSearchFilterService::default();
    mock_filter_service
        .expect_match_user_search_filters()
        .return_once(move |_| Box::pin(async move { Ok(vec![summary_clone]) }));
    mock_filter_service
        .expect_find_search_filter_product_match()
        .return_once(|_, _, _, _| Box::pin(async { Ok(None) }));

    // Delegate quota counting to the real count (DynamoDB-backed)
    let real_count = match_count;
    mock_filter_service
        .expect_count_user_search_filter_matches_for_this_month()
        .return_once(move |_| Box::pin(async move { Ok(real_count) }));

    let mut mock_get_product_service = MockGetProductService::default();
    mock_get_product_service
        .expect_find_product()
        .return_once(move |_, _| Box::pin(async move { Ok(product_clone) }));

    let mock_enhanced_service = MockEnhancedSearchMatchService::default();

    // Process the product event through the ProductMatcherService
    ...
    let event = Event {
        ...
        payload: ProductEventPayload::ProductDomainEvent(
            product::core::product_event::domain::ProductDomainEventPayload::StateChanged(
                product::core::product_event::domain::ProductStateChangeDomainEventPayload {
    ...
    let result = product_matcher_service
        .process_product_event(event)
        .await
        .expect("process_product_event should succeed");

    // A match record IS created even though quota is exceeded
    assert_eq!(
        result.matches.len(),
        1,
        "Expected a match to be created even when quota is exceeded"
    );
    assert_eq!(result.matches[0].user_id, user_id);
    assert_eq!(result.matches[0].user_search_filter_id, new_filter_id);
    assert_eq!(result.matches[0].shop_id, product.shop_id);
    assert_eq!(result.matches[0].shops_product_id, product.shops_product_id);

    // But NO notification command is created
    assert!(
        result.notification_commands.is_empty(),
        "Expected no notification commands when user has exceeded their quota"
    );
}
