...
describe.each(STAGES)("%s compute eventing", (stage) => {
  test("retains partner consumers while ProductListing and CDC consumers are off by default", () => {
    ...
    const translationActivation = { "Fn::If": ["ProductTranslationConsumerActivation", true, false] };
    ...
    expect(templateJson.Parameters.PartnerIntegrationEnabled).toMatchObject({
      Type: "String",
      Default: "false",
      AllowedValues: ["true", "false"],
    });
    expect(templateJson.Conditions.PartnerIntegrationActivation).toEqual({
      "Fn::Equals": [{ Ref: "PartnerIntegrationEnabled" }, "true"],
    });
    if (stage === "ephemeral") {
      expect(templateJson.Parameters.FxRateRefreshEnabled).toBeUndefined();
      expect(templateJson.Conditions.FxRateRefreshActivation).toBeUndefined();
    } else {
      expect(templateJson.Parameters.FxRateRefreshEnabled).toMatchObject({
        Type: "String",
        Default: "false",
        AllowedValues: ["true", "false"],
      });
      expect(templateJson.Conditions.FxRateRefreshActivation).toEqual({
        "Fn::Equals": [{ Ref: "FxRateRefreshEnabled" }, "true"],
      });
    }
    ...
    expect(templateJson.Parameters.ProductTranslationConsumerEnabled).toMatchObject({
      Type: "String",
      Default: "false",
      AllowedValues: ["true", "false"],
    });
    expect(templateJson.Conditions.ProductTranslationConsumerActivation).toEqual({
      "Fn::Equals": [{ Ref: "ProductTranslationConsumerEnabled" }, "true"],
    });
    ...
    expect(shopifyMapping?.Properties).toMatchObject({
      BatchSize: 10,
      Enabled: partnerIntegrationActivation,
    ...
    const translationMapping = mappings.find((mapping) =>
      JSON.stringify(mapping.Properties?.FunctionName).includes("ProductTranslationVersion"),
    );
    expect(translationMapping?.Properties).toMatchObject({
      BatchSize: 1,
      Enabled: translationActivation,
      FunctionResponseTypes: ["ReportBatchItemFailures"],
    });
    expect(JSON.stringify(translationMapping?.Properties?.EventSourceArn)).toContain(
      stage === "ephemeral"
        ? "WorkerQueuesProductTranslationQueue"
        : `aura-worker-product-translation-${stage}`,
    );
